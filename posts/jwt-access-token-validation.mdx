...
| Check | Description |
| ----- | ----------- |
| <span style={{color:'green'}}>**Algorithm**</span> | The API configures the algorithm(s) which which it expects JWTs to be signed. |
| <span style={{color:'green'}}>**Signature**</span> | The API provides the token signing public key to the library. The most common way to do so is to configure a trusted JWKS URI. |
| <span style={{color:'green'}}>**Issuer**</span> | The API configures its expected issuer to be the identity of the authorization server. |
| <span style={{color:'green'}}>**Audience**</span> | The API configures its expected audience to be a logical identifier that represents its own identity. |
| <span style={{color:'green'}}>**Time**</span> | The API only accepts JWTs that are valid for use at the current time and rejects expired JWTs. |
...
| ----- | -------------- |
| <span style={{color:'green'}}>**Same Key Identifier**</span> | The security library uses the cached JSON Web Key, to prevent further calls to the authorization server’s JWKS URI. |
| <span style={{color:'green'}}>**New key identifier**</span> | The security library makes a new call to the JWKS URI to get updated JSON Web Keys, including that for the new key identifier. |
...
After JWT access token validation completes, the API trusts claims in the JWT payload and uses them in its business authorization. I say more about the use of claims in this blog’s <Link href='/posts/api-authorization-design'>Authorization Behaviour</Link> post.
...
- I explain this blog's <Link href='/posts/oauth-infrastructure-setup'>OAuth Infrastructure Setup</Link>
