# Final SPA – Code Overview

Previously I explained <Link href='/posts/how-to-run-the-react-js-spa'>How to Run the Final SPA</Link>, which uses cookies to transport short-lived access tokens to APIs. Next I describe the main changes to the SPA's code, with a particular focus on API requests. See also the <Link href='/posts/api-journey-client-side'>Client Side API Journey</Link> for a summary of the requirements being met.

### Previous Samples

I provided two earlier plain TypeScript SPA and API code samples. These SPAs used access tokens in the browser, which is no longer recommended.
