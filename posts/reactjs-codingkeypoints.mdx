...
Previously I explained <Link href='/posts/how-to-run-the-react-js-spa'>How to Run the Final SPA</Link>, which uses cookies as API credentials. Next I describe the main changes to the SPA's code, with a particular focus on API requests. See also the <Link href='/posts/api-journey-client-side'>Client Side API Journey</Link> for a summary of the requirements being met.

### Previous Samples

I provided two earlier plain TypeScript OAuth code samples. These used access tokens in the browser, which is no longer recommended.

| Solution | Description |
| -------- | ----------- |
| <Link href='/posts/basicspa-codingkeypoints'>Code Sample 1</Link> | Authorization code flow with PKCE, calling APIs with access tokens and handling 401 responses. |
| <Link href='/posts/updated-spa-coding-key-points'>Code Sample 2</Link> | Added in-memory token storage, silent token renewal and logout, to complete the SPA's session management. |

### SPA Code

The final SPA is a React app that runs at a */spa* path. Additional paths, for other micro-frontends, could run within the same web domain, to keep the code size manageable as it grows:
...
The final SPA could use any web static content host, but this component is always lightweight and never interferes with the frontend experience. For development I use the full-featured Express HTTP server to test production web behaviour locally, such as the content security policy. The web host's logic serves the SPA’s static content from the */spa* folder. Other folders could be added in future, for additional micro-frontends:
...
React manages rendering when *state* changes, and views are created with *props*. I also like to use *view models* to handle operations such as API calls, and to keep the view classes focused on presentation:
...
Views are recreated frequently, whereas I manage application state within view model classes that get created only once. I manage this by storing child view model references within the root *AppViewModel*:
...
I use the React framework’s hook-based syntax to develop the SPA’s views, as shown below. In mobile development I use other frameworks, such as SwiftUI, which have their own view syntax.
...
I avoid the use of frameworks like React or SwiftUI for general logic, including API and OAuth requests. This leads to more natural object-based code that is easier to port across technologies.

### Triggering API Requests

For views that call APIs, data loading typically begins when the [useEffect hook](https://reactjs.org/docs/hooks-effect.html) executes. The view’s *startup* method calls a *loadData* method to triggers calls to APIs. The *Reload Data* button also invokes the *loadData* method.
...
The view model code is business-focused and uses a *fetch* client to deal with lower level aspects. The *ViewModelCoordinator* object provides a mechanism to trigger a new login redirect when the API session expires:
...
Now that you understand that API requests run concurrently, let's see how the SPA initially calculates that it must initiate user authentication, to get an access token with which it can call APIs.
...
When the login response is received, the page load handler processes the login response. The result is to issue encrypted session cookies (which contain the underlying OAuth tokens) to the browser.
...
The lower level API code uses a basic caching layer. This avoids duplicated API calls, such as when the user navigates to an already visited view, or uses the back button. The caching layer also deals efficiently with re-entrancy when views are recreated, such as when you use React strict mode.

### Synchronized Token Refresh

The SPA uses a class called *ConcurrentActionHandler* to avoid sending multiple token refresh requests at the same time. This prevents potential reliability problems when the authorization server issues single-use refresh tokens. The SPA only sends a physical token refresh request for the first view that requests a refresh. All other views wait, then receive the same result as the first view. I manage this using a technique of queueing up promises:
...
The final SPA deals reliably with some tricky areas, in the lower-level plumbing classes. Some of these could be turned into a shared library, which would reduce code. In a real organization the goal would be to grow the web code base by adding new view and view model classes. A security-first design, with an early focus on code setup and deployment, helps to enable this.

There is one remaining post for this blog's final SPA. It is part of the cloud theme and covers <Link href='/posts/cdn-static-content-delivery'>Web Content Delivery</Link> using a cloud platform. The SPA's static content gets uploaded to a content delivery network which deploys it to ~400 global locations for globally equal performance. Ultimately though, the SPA behaves exactly the same on a development computer as it does in any deployed system.
...
- I begin this blog's second theme with an <Link href='/posts/api-platform-design'>API Platform Overview</Link>.
