...
The previous post provided an <Link href='/posts/updated-api-coding-key-points'>API Code Details</Link> for the second code sample. Next I port the code sample to [Microsoft Entra ID](https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id) and use the same libraries in the SPA and API.
