...
In the previous post I provided an <Link href='/posts/updated-api-coding-key-points'>API Code Overview</Link> for the second code sample. Next we will prove that the current code sample is a portable OAuth implementation by using a different authorization server. This will be [Microsoft Entra ID](https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id) and will run in the Microsoft Azure cloud.
