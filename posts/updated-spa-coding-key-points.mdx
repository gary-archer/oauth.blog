...
In the previous post I described the behaviour of the <Link href='/posts/improved-spa-code-sample-overview'>Updated SPA and API Code Sample</Link>. Next I summarize some changes to the SPA code, with a particular focus on <Link href='/posts/user-sessions-and-token-renewal'>Iframe-Based Access Token Renewal</Link> and its difficulties.
...
### Enabling Iframe-Based Access Token Renewal
...
When using refresh tokens, the refresh operation should take a short time such as 100 milliseconds and users should not notice any time overhead. Yet when you use iframe based token refresh, this time can take considerably longer and has greater user impact.

### Access Token Refresh Before Expiry

There is a much simpler way to implement token refresh, by updating the *oidc-client-ts* settings to those shown here. A background timer then attempts to refresh the access token by comparing the current time to the *expires_in* field from the token response. The time for iframe-based token refresh occurs in the background so you avoid impacting users.
...
Your API logic can then interpret any 401 returned from APIs as end of session and redirect the user to-reauthenticate. You should also consider adding logic to <Link href='/posts/basicspa-codingkeypoints#preventing-redirect-loops'>prevent redirect loops</Link> to deal with conditions if APIs permanently reject access tokens.
...
Yet local storage is also the <span style={{color:'red'}}>least secure way to manage access tokens in the browser</span>, with greatest XSS threats.

### The SPA OAuth Journey

When you use the traditional SPA flow using only JavaScript code, you typically undergo a journey like this:

- You first store access tokens in local storage, to prevent losing them during page reloads and multi-tab browsing.
- You then realise this is bad from a security viewpoint and try to use in-memory access token storage to improve security.
- You must then manage page reloads and multi-tab browsing, and token refresh becomes considerably more difficult.
- You then realise that even with very careful code, your SPA still has security and reliability issues.

Eventually, you come to realise that to solve the security, usability and reliability conditions, you need a backend component that assists the SPA with its security and also serves the SPA to ensure the best usability and reliability. You can realise such a solution with a correctly designed <Link href="/posts/spa-back-end-for-front-end">Backend for Frontend</Link> as I show later in this blog. An important part of this design is error responses that put the SPA in control.
