...
Previously I explained some <Link href='/posts/logout'>Logout</Link> behaviors. I finish the discussion on SPA session management by describing the role of ID tokens, which are designed to be read by client applications rather than APIs.
