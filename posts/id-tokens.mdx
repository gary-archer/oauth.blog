...
Previously I explained some <Link href='/posts/logout'>Logout</Link> behaviours. I finish the discussion on SPA session management by describing the role of ID tokens, which are designed to be read by client applications rather than APIs.
...
An earlier post explained some <Link href='/posts/basicspa-oauthworkflow'>Initial OAuth Messages</Link>. Clients receive an authorization code in a front channel browser response, then send in a back channel HTTPS POST to the token endpoint of the authorization server, to swap the code for tokens.
...
I summarized a few points about ID tokens, which provide verifiable proof of the authentication event to clients. All modern secured frontends should receive an ID token as part of using up-to-date security.

### Next

- I explain this blog's <Link href='/posts/improved-spa-code-sample-overview'>Updated SPA and API Code Sample</Link>.
