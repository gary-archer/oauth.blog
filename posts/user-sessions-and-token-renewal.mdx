...
You can solve some usability and reliability problems by lowering security, yet these provide least protection against XSS threats, which can easily read your tokens and may be able to send them to a malicious host that calls your APIs. This blog avoids these options:

| Behavior | Description |
| -------- | ----------- |
| <span style={{color:'red'}}>Access Tokens in Local Storage.</span> | If you store access tokens in local storage, you no longer need to perform a token refresh when the user reloads the page or opens a new browser tab. |
| <span style={{color:'red'}}>Refresh Tokens in Local Storage.</span> | If you store refresh tokens in local storage, you can the use of SSO cookies and get a new access token with a simple Ajax request from JavaScript. |
...
The <Link href='/posts/updated-spa-coding-key-points'>Updated SPA – Code Overview</Link> explains some code complexities if you attempt to implement iframe-based access token renewal with in-memory storage. Even with careful code there are security, reliability and usability issues due to cookie issues.

By default, the second code sample does not work at all with AWS Cognito. To provide some kind of working demo solution I fall back to using refresh tokens stored in-memory for that authorization server. Usability is intentionally suboptimal, and there is a new OpenID Connect redirect (a single-sign-on event) every time you reload the page or open a new browser tab.
