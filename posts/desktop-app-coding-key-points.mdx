...
Previously I summarised some test behavior in <Link href='/posts/desktop-apps-overview'>How to Run the Desktop Code Sample</Link>. I complete the content for the initial desktop app with some code-level details.

### Code Layout

Electron apps have two processes, [Main and Renderer](https://www.electronjs.org/docs/latest/tutorial/process-model) where the renderer process is an SPA frontend and the main process is a backend that uses Node.js. The folder structure of the desktop app reflects this, with *main*, *renderer* and *shared* folders. The views within the renderer process use plain JavaScript and are the same as those from this blog's <Link href=''>Second SPA</Link>.
...
![bundles](/images/430/bundles.jpg?v=20240922)
</div>

The main and renderer processes use independent webpack builds and independent *tsconfig.json* files. Therefore, the main bundle does not contain any view-specific code and the renderer bundles does not contain any of the desktop app's backend code. The main process uses the following webpack configuration:
...
The Electron app follows Electron's [Security Best Practices](https://www.electronjs.org/docs/latest/tutorial/security) so that the renderer process runs with least privilege. It therefore does not have permissions to perform operations like running an HTTP server or opening the system browser. Instead, to initiate this type of operation, the desktop app uses Electron's [Inter-Process Communication](https://www.electronjs.org/docs/latest/tutorial/ipc). I also proxy API requests via the backend to keep tokens out of the Chromium browser. The IPC code starts with registering some event names in the shared code:
...
The frontend uses a *RendererIpcEvents* entry point for sending IPC events. OAuth and API operations are forwarded to the main process:
...
The following main classes are used to handle OAuth and API logic. The strategy is to drive behaviour from the renderer process so that the frontend is in control. The interplay is a little intricate, since the app also handles many error conditions. Since the frontend only uses forwarders, this post only explains the backend classes.

| Class | Responsibilities |
| ----- | ---------------- |
| <span style={{color:'green'}}>**ApClient**</span> | A forwarder to initiate API operations. |
| <span style={{color:'green'}}>**ApiService**</span> | Does the main work to send API requests with access tokens to the remote API endpoints. |
| <span style={{color:'green'}}>**AuthenticatorClient**</span> | A forwarder to initiate login and token-related operations. |
| <span style={{color:'green'}}>**AuthenticatorService**</span> | Does the main work to integrate with the AppAuth-JS library to trigger login and token requests, then handle responses. |

### Debugging HTTP Requests

One consequence of this design is that you cannot easily view HTTP requests initiated by the renderer process. To do so, you can use an <Link href='/posts/http-debugging-setup'>HTTP proxy Tool</Link>. The easiest option is to run the tool and change your network settings, then edit the *desktop.config.json* file to activate the proxy, then run the desktop app with the following commands:
...
The code for the renderer process begins by creating an *App* class, which acts as the application shell. This creates some global forwarder objects that drive API calls and OAuth operations, in the same manner as earlier SPAs:
...
When there is no token yet or token refresh fails with an *invalid_grant* error, views receive a *login_required* error and terminate API requests in a controlled manner. This error code is treated specially and results in a redirect to the *Login Required View*, which is given a return location to use once login completes:
...
The *LoginAsyncAdapter* translates from the older callback syntax that AppAuth-JS uses to a modern async-await syntax. I use some other utility classes, including a *BrowserLoginRequestHandler*, which receives the authorization response, so that I have best control over technical behaviors, including error handling and the post-login browser display.
...
The initial desktop app simply stores the tokens in-memory. Every time the app is restarted, the user is prompted to re-login. I show how to improve this user experience in the final desktop code sample.
...
- We must resume the login response with the correct state and code verifier.
...
The *AuthenticatorService* also has some test methods to enable us to simulate token expiry responses. As for the earlier SPA, the desktop app simply adds text to corrupt the token values sent to the API. This mechnism helps to ensure that the app reliably handles expiry conditions.
...
- I provide a <Link href='/posts/final-desktop-sample-overview'>Final Desktop Code Sample</Link> with some additional behaviors.
