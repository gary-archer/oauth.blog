...
Previously I summarised this blog’s <Link href='/posts/api-platform-design'>API Platform Overview</Link> to express some essential high-level ingredients. Next I summarise some end-to-end technical behaviors for OAuth-secured APIs, which I implemnt in this blog’s final API code samples, in multiple technology stacks:
...
APIs must collect values for their business authorization. This code often requires more data than just the token claims, to enforce fine-grained business permissions. There are various ways in which you could manage this requirement. In this blog I show how to extend OAuth middleware to build a useful claims object. You can read more about the approach in the <Link href='/posts/api-authorization-design'>API Authorization Behaviour</Link> blog post.
...
You should test OAuth API security frequently as part of a secure API development lifecycle. This blog’s final APIs use tests that <Link href='/posts/api-automated-tests'>mock the authorization server</Link>. This technique enables you to easily control access tokens when you write integration tests on a development computer. I use the same JOSE library for both API access token validation and test code that produces JWTs.
...
The logs enable <Link href='/posts/api-technical-support-analysis'>people-focused technical support queries</Link> to measure errors or slowness. For example, you can produce a breakdown of errors by type and frequency for a time period. You can then iron out any reliability issues as part of your quality processes.
...
![spa and api](/images/260/spa-and-api.jpg?v=20240913)
...
- Next I take an equivalent look at the <Link href='/posts/api-journey-client-side'>API Journey – Client Side</Link>.
