...
The desktop app re-uses some code from this blog’s <Link href='/posts/improved-spa-code-sample-overview'>second SPA</Link> and has exactly the same views:
...
Before issing the redirect the the desktop app runs an HTTP server within its own process. The desktop app uses a low privilege hostname and low privilege port (above 1024), to ensure that users do not require administrator privileges.
