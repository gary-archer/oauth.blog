...
![repo](/images/420/repo.jpg?v=20240922)
...
If you did an earlier SPA login in the same browser and used the save password feature, you can password autofill for the desktop app. After user login, the system browser renders a post-login page, rather than showing a blank page. Alternatively, you could redirect the browser to an external site if your prefer.

![browser logged in](/images/420/browser-loggedin.jpg?v=20240922)

When login completes, the app gets OAuth tokens, stores them in memory and returns itself to the foreground. It then gets data from a <Link href='/posts/serverless-api-overview'>cloud hosted OAuth-secured API</Link> and renders results.
...
Before issing the redirect the the desktop app runs an HTTP server within its own process. The desktop app uses a low privilege hostname and port (above 1024) when doing so, to avoid users needing administrator privileges or the operating system prompting the user for permissions.
...
A busy user could fail to complete a login and close the browser window, leaving the desktop app in a *login in progress* state. Our app allows the user to retry the login, by clicking the *Sign In* button again.
