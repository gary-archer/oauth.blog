...
Previously I provided a brief <Link href='/posts/desktop-apps-overview'>Desktop Code Sample Overview</Link>. Next I explain how to run a more detailed desktop code sample on your local computer, which manages OAuth behaviors in a more complete way.
...
The desktop app loads its home page and prepares to call the API, then detects that there is no OAuth access token, so the *Login Required View* is presented:
...
An authorization redirect is triggered in a separate system browser window, and you can use this blog’s test user account to sign in:

- User: *guestuser@example.com*
- Password: *GuestPassword1*

If you did an earlier SPA login in the same browser and used the save password feature, you can password autofill for the desktop app. After user login, the system browser renders a post-login page, rather than showing a blank page. Alternatively, you could redirect the browser to an external site instead.

![browser logged in](/images/420/browser-loggedin.jpg?v=20240922)

When login completes, the app gets OAuth tokens, stores them in memory and returns itself to the foreground. It then gets data from a <Link href='/posts/serverless-api-overview'>cloud hsoted OAuth-secured API</Link> and renders resutls:
...
| Setting | Description |
| ------- | ----------- |
| <span style={{color:'green'}}>**Loopback Host Name**</span> | The hostname for the loopback HTTP server. |
...
The desktop app is registered as an OAuth client in AWS Cognito, with the following settings:

![oauth client](/images/420/oauth-client.jpg)

According to RFC 8252, it should be possible to register a redirect URI that begins with *http://127.0.0.1* and the client can then use any port at runtime. Yet some authorization servers do not support this. AWS Cognito required the *http://localhost* base URL instead of the loopback IP address, and I also had to register all possible redirect URI values.
...
Before issing the redirect the the desktop app runs an HTTP server within its own process. The desktop app uses a low privilege hostname and port (above 1024) when doing so, to avoid users needing administrator privileges or the operating system presenting alerts.

### Step 8: Login Completion Page

I uploaded the login completion page to an AWS S3 Bucket and deploy it at an HTTPS URL using the AWS CloudFront distribution for this blog's final SPA:
...
### Step 10: Test Login Re-Entrancy

A busy user could fail to complete a login and close the browser window, leaving the UI in the following state. Our app allows the user to provide a gesture to retry the login, by clicking the *Sign In* button again:

![re entrancy](/images/420/re-entrancy.jpg)

### Step 11: Simulate Expiry Events
...
The initial desktop app implements only a basic logout, that discards its tokens and returns it to the *Login Required* view. The next login uses the OpenID Connect *prompt=none* parameter to force a new login. In a real organization this would enable you to login as a different test user. However, AWS Cognito does not support this parameter, so this behavior does not work as intended.
...
- I explain some <Link href='/posts/desktop-app-coding-key-points'>Desktop App Code Details</Link>.
