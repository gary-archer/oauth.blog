...
| Quality | Description |
| ------- | ----------- | 
| <span style={{color:'green'}}>**Secure**</span> | Client call APIs using message credentials containing user attributes that APIs use to protect business data. |
...
This blog is code-centric with a focus on modern code that uses a separation of concerns. I provide a number of [Code Samples](https://github.com/gary-archer) which I use to stay up to date with mainstream technology stacks. The code is intentionally moderately complex, with a fair amount of plumbing to deal with foundational behaviours. At a real organization, a technical goal would be a clean code setup that enables the valuable business logic to scale.
