...
In the previous post I explained <Link href='/posts/developer-ssl-setup'>SSL Trust Configuration</Link> for a development computer. Next, I summarize some practicalities of user management in an OAuth Architecture. In many setups identity data and business data need to work together.
...
You may also need to call the authorization server’s user management APIs for other use cases, as summarized in the following table. In at least some cases, you may need to design how to save attributes to both the identity data and to business data:

| Use Case | Description |
| -------- | ----------- |
| <span style={{color:'green'}}>**Existing User Migration**</span> | A migration process reads existing users in bulk from an existing data source and imports them into the authorization server. |
...
| <span style={{color:'green'}}>**Self-Sign-Up**</span> | A user signs up and provides proof of their identity, after which attributes save to both data sources. |
| <span style={{color:'green'}}>**Profile Edits**</span> | A user edits their profile in an application form and data is saved to both data sources. |
...
- I explain this blog’s <Link href='/posts/api-authorization-design'>API Authorization Behaviour</Link>.
