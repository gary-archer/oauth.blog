...
You will then be able to navigate to the following URL in a browser without any SSL trust warnings:
...
You can run any of this blog's final frontend clients against the Node.js API:
...
The overall behaviour should be to deal with access tokens correctly and then set up the API's business logic with the values it needs for its business authorization. These values may originate from multiple sources and not just the access token:
...
The API uses cloud-native environment and includes some Docker and Kubernetes deployment resources. A basic deployment can be run with these commands:
