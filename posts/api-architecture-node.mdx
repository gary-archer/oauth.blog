...
The OAuth integration and its claims handling uses exactly the same techniques as the second code sample, summarised earlier in its <Link href='/posts/updated-api-coding-key-points'>API Coding Key Points</Link>. Therefore I will not repeat them, and these posts will instead only focus only on newer features.
