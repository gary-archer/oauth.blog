...
The OAuth integration and its claims handling uses exactly the same techniques as the second code sample, summarised earlier in its <Link href='/posts/updated-api-coding-key-points'>API Code Overview</Link>. Therefore I will not repeat them, and these posts will instead only focus only on newer features.
...
- Next we will take a look at the <Link href='/posts/final-nodeapi-coding-key-points'>Node.js API Code Overview</Link>
