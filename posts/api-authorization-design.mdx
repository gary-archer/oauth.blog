...
A client with an access token that contains only a *sales* scope can never successfully access data from *finance* API endpoints. The API would immediately reject the access token with a 403 forbidden error. This is true even if the user has finance rights. Therefore, scopes enable you to set boundaries on where a client can use access tokens.
...
In this blog I use a fictional *investments* scope and frontend clients need access tokens with this scope to call any endpoint of my example APIs. More generally, scopes represent data and can be hierarchical. Scopes are also useful when you want to get a higher privilege access token. For example, before performing a money transfer you might send an authorization redirect requesting a high privilege scope such as *investments:payment*. A high privilege API endpoint could require that scope. 

You don't need many scopes since you use claims for most authorization. Yet you should design scopes in terms of end-to-end flows. If multiple APIs call each other, they should each validate an access token and implement business authorization. When a user is present, each API should receive the user identity and its required scopes and claims in its access token. The source API can send its current access token or use OAuth token exchange to get a new one from the authorization server with different scopes. 
...
You can issue different claims to access tokens per user and they are more flexible than scopes. You might consider issuing a scope like *investments:read*, though this is often insufficient, since users often have read-only access to some data and read-write access to other data. Use claims to represent those types of rules. Claims can be primitive values, arrays or objects.

In some cases, APIs cannot decide how to authorize solely from a user ID in an access token. For example, an access token with an *investments:payment* scope might also include payment-related claims from third-party systems that describe the payment, which an API endpoint needs to use to authorize the payment correctly.
...
You should manage volatile business permissions in your APIs rather than your authorization server. You usually only need to issue a few custom claims to access tokens. These values should be stable and should not change within a user's authenticated session. When your API authorizes requests it can derive volatile business permissions from other claims in the access token, like a user ID or role.

Therefore, APIs often need to combine fine-grained business permissions with claims from the access token. As for your scopes design, when you get a good balance you use stable access tokens and you reduce the need to manage access tokens versioning. You can deploy APIs frequently without needing to redeploy or reconfigure the authorization server.

### Customizing the Claims Principal

In your API code, you may therefore need to combine access token claims with extra values. You can do this in multiple ways, according to your coding preferences. When I first wrote this blog, the AWS Cognito authorization server did not support issuing custom claims to access tokens, yet I still wanted a useful and expressive claims principal. Therefore I designed an object with the following structure:
...
My code samples continue to use this technique, to show how to take closer control of claims. I perform JWT validation on every request and then look up extra values from a trusted source - like the API's database - before creating the claims principal. The business logic classes then receive a useful object with which to implement their authorization:

![token to claims](/images/130/token-to-claims.jpg)

This code setup is one way to ensure that your API's business logic receives future changes to volatile business permissions. Understand though, that true claims are those that the authorization server asserts when it cryptographically signs access tokens. The main identity values should always be issued to access tokens when possible, so that the API receives them in a cryptographically verifiable format. 
...
The API authorizes access to resources using both the *role* from the access token and the *regions* array from the extra claims. If business authorization rules evolve, the API can update the extra claims object updates to the authorization server.
