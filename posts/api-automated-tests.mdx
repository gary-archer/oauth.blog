...
In the previous post we described the <Link href='/posts/spring-boot-api-coding-model'>Java Spring Boot API Code Overview</Link>. Next we will describe one way to test OAuth secured APIs, with a focus on *Developer Productivity*.
