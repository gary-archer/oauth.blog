...
Previously I provided a <Link href='/posts/log-aggregation-setup'>Log Aggregation Setup</Link> to deploy tooling that enables log queries. This post shows how different technical stakeholders could then query the log aggregation system productively to support APIs. You could enable such a solution with this blog's <Link href='/posts/effective-api-logging'>Effective API Logging</Link> design.

If you followed the logging setup you are running an API and an API client. You can use the client to initiate multiple requests to the API, then use the Kibana system to query the API logs you generate. The main focus of this post is control over log data, so I only use the *Dev Tools* console. I consider log queries for three roles of log user: *DevOps*, *Testers* and *Developers*.
...
For this type of query, the output format in the Kibana frontend is a tabular list of columns:
...
Support staff can then query *how often* this type of error has occurred and for *which users*:
...
This can provides powerful ways to enable DevOps staff to resolve code-level problems, and enable qualities like DevOps resolving <span style={{color:'red'}}>80% of production incidents</span> without the need to involve a specialist developer.
...
Sometimes APIs are coded with concurrency bugs. This blog's final APIs include a <Link href='/posts/api-automated-tests'>basic load test</Link> that fires many API requests in concurrent batches and intentionally send some requests with invalid input, to rehearse API errors:
...
API developers are sometimes *on call* to help resolve production incidents. You take most of the pain out of this process when you do early rehearsal of failure scenarios. The <Link href='/posts/error-handling-and-supportability'>Error Handling and Supportability</Link> post explains some techniques.
...
- I begin a new theme by discussing <Link href='/posts/native-architecture-goals'>Native Architecture Goals</Link>.
