...
In AWS Cognito the deployed SPA is registered as follows, to point to an AWS CloudFront web origin. The SPA is now registered as a confidential client, and its backend for frontend uses a client credential in grant requests for tokens:
