...
Browsers often inherit SSL trust from the above operating system default trust stores, though there are some exceptions. On Firefox, you must navigate to *Options / Privacy & Security / Certificates / View Certificates / Authorities / Import* to import a root certificate. You must also do so if you run Google Chrome on Linux. When you add trust, select the option to *Trust this CA to identify websites*.
