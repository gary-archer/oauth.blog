# Updated API – Code Overview

The previous post I provided an <Link href='/posts/updated-spa-coding-key-points'>SPA Code Overview</Link> for the second code sample. Next I summarize the main API code changes.

### API Authorization Implementation

In the first sample’s <Link href='/posts/basicapi-codingkeypoints'>API Code Overview</Link> we explained that OAuth processing consists of the following two main code areas:
...
The API’s claims are customized to include additional finer-grained authorization values:
