...
The <Link href='/'>Home Page</Link> described OAuth in terms of overall goals for a software platform. In this post I explain qualities to look for in an optimal web architecture, which is not just about security. This post briefly summarises some goals and concerns for web apps.
...
### Productive Front End Development
...
| <span style={{color:'green'}}>**Portable Hosting**</span> | You should only need to upload web static content to a CDN to deploy SPAs, so avoid designs that prevent this. |
...
I articulated some desired behaviour, and next I provide a code example to get started on the journey of achieving it.

### Next

- I provide an <Link href='/posts/basicspa-overview'>Initial SPA Code Sample Overview</Link>
