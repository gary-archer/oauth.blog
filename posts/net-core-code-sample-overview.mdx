...
Previously I explained the <Link href='/posts/final-nodeapi-coding-key-points'>Final Node.js API Code Details</Link>, including some behaviours from this blog’s <Link href='/posts/api-journey-server-side'>API Journey – Server Side</Link> blog post. Next I provide an equivalent API in C# and .NET.
...
To prevent SSL trust problems from tests or the browser you should <Link href='/posts/developer-ssl-setup'>Configure .NET and Browser Trust</Link> for the root certificate at the following location. 
...
- I explain this blog's <Link href='/posts/net-core-api-oauth-integration'>.NET API OAuth Integration</Link>.
