...
To do so, follow the instructions to run the client against a local API, for example by using a *LOCALAPI* environment variable to override the default behavior of a cloud API. When the frontend runs a code flow you can authenticate with this blog's test credential:
