...
The following screenshot shows the test output. API developers could publish categorized security test results to stakeholders to help ensure that all important security conditions are covered by tests and frequently verified.
...
| Client ID | The name of the load test client, as a mechanism to find all load test sessions. |
...
I enabled productive testing of this blog's final APIs, as a standalone component, using mock OAuth infrastructure. The testing also verifies that the API's error handling and logging behaviours work as expected when there are concurrent API requests.
