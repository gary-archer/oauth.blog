...
Upon return from login, the Chrome Custom Tab hosts an intermediate page. This page receives the authorization response, presents a continue button to ensure a user gesture and then invokes the login receiver activity with the authorization response's query string:
...
The app receives logout responses on its *Post Logout Redirect URI* using the same intermediate web page mechanism that logins use. The Android manifest file includes an extra activity that enables the app to receive logout responses from the intermediate web page.
