...
The big change from a web development viewpoint is that SPAs should use *HttpOnly SameSite=strict Secure* cookies to transport access tokens to APIs. The cookie requirement is an awkward one for SPAs, where you want to continue to use the modern web and its benefits, rather than return to older website technology stacks.
...
The key technique to maintain an SPA architecture and meet all three requirements is to use API-driven cookie issuing, The *Token Handler Pattern* is such an approach, which uses the following API components to implement cookie security. These components do not need to run on development computers. They are also stateless and easy to manage, since cookies contain all state.
...
- **Reverse Proxy**. Development setups and deployments use a reverse proxy that implements security. When requests for HTML files do not have a session cookie the browser is abrubtly redirected. In some implementations, fetch requests may also experience abrupt redirects. Some reverse proxies may not even support forwarding access tokens to APIs. This type of solution loses technical control.

- **Website**. Developers run a complex local setup where their web host comes into contact with sensitive API data and typically uses a website cookie framework. The web host may use server-side rendering to return HTML and data together. It requires a runtime and other server dependencies that complicates developer setups and limits deployments of web static content.
...
The following diagram illustrate the overall components, where the token handler acts as an advanced reverse proxy that runs in a subdomain in the same site as the SPA's web origin. Calls from the SPA to APIs use the latest and most secure first-party cookies.

![components](../images/210/components.jpg)

I deploy the final SPA's web content as an [AWS CloudFront Distribution](https://aws.amazon.com/cloudfront/) that delivers web static content. Neither the SPA nor APIs require any cookie-related code. The API uses exactly the same code to handle requests from web, desktop and mobile clients.
...
### This Blog's Token Handlers

For this blog, I use two types of token handler, whose code you can study:

- When the SPA connects to low cost AWS lambda APIs, it uses a [Serverless Token Handler](https://github.com/gary-archer/oauth.tokenhandler.serverless) that also runs as a lambda function.
- When the SPA connects to cloud native APIs, it uses a [Cloud Native Token Handler](https://github.com/gary-archer/oauth.tokenhandler.cloudnative) that integrates with a cloud native API gateway.

A token handler externalizes cookies and CORS responsibilities from both web and API components. It also gives SPAs additional deployment choices. You can easily split a large SPA into micro frontends, that run at different paths, and share the same API-issued cookies. Alternatively, you can run multiple SPAs with token handlers, on distinct domains, to isolate cookies between SPAs.

### Where Are We?

I explained how I used URLs to design deployments for this blog's final SPA. Next, I explain this blog's final SPA, which uses cookies to call APIs, to both harden its security and to solve its usability problems.
