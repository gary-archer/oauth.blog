...
In the previous post I ran a <a href='azure-ad-troubleshooting'>Microsoft Entra ID SPA and API OAuth Flow</a>. This blog's example SPA now needs hardening before it is final, to follow security best practices in the browser and to minimize threats. The SPA must also resolve its <a href='user-sessions-and-token-renewal'>Token Renewal Usability and Reliability Issues</a>.
...
- I provide a <a href='final-spa-overview'>Final SPA</a> that uses cookies to call APIs.
