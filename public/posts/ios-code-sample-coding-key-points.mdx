...
Upon return from login, the ASWebAuthenticationSession window hosts an intermediate page. This page receives the authorization response, presents a continue button to ensure a user gesture and then invokes the login receiver activity with the authorization response's query string:
...
The app receives logout responses on its *Post Logout Redirect URI* using the same intermediate web page mechanism that logins use.
