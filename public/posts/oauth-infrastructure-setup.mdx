...
When working with OAuth, I recommend designing URLs for a development computer, so that you operate similarly to deployed environments. I use the following base URLs for this blog's code samples on a development computer:

| Development Base URL | Description |
| -------------------- | ----------- |
| *https://www.authsamples-dev.com* | A base URL for web applications |
| *https://api.authsamples-dev.com:446* | A base URL for APIs |
...
Components that run locally use the fictional domain *authsamples-dev.com*. This is an alias to your local loopback address of *http://127.0.0.1*. You configure subdomain aliases in your local computer's *hosts file*, at one of these locations:
...
For the first code sample, you configure the following subdomains.
...
By default, this blog's code samples use my low cost AWS Cognito authorization server at *https://login.authsamples.com*. Anyone can run frontend code samples that point to it, then sign in and run an OAuth application lifecycle.
...
Frontend apps are registered as OAuth clients using an administration user interface, where you configure fields like the *Client ID* and *Redirect URI*.

![oauth client](../images/80/oauth-client.jpg?v=20251230)
...
### Step 10: Build Portable Apps that use OAuth Standards

A key objective should be to use security libraries so that you integrate OAuth in the following areas in a standards-based and portable manner. In this blog I never use vendor-specific libraries from a particular authorization server provider:

| Area | Description |
| ---- | ----------- |
| Clients | Standards-based code flow, to authenticate users and get an access token to enable API access. |
| APIs | Standards-based access token validation, to authenticate requests and get values for authorization. |

### Step 11: Clarify Requirements

During the journey of integrating OAuth into apps, you will form a better idea of what you need from the authorization server. Before committing to an implementation, ensure there are no blocking issues in your end-to-end flows, in areas such as values you can store against user accounts, values you can issue to access tokens, or the login user experience.
