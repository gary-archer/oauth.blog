...
The desktop app re-uses some view-related code from this blog's <a href='improved-spa-code-sample-overview.mdx'>Updated SPA</a>:
...
Once user authentication completes, the system browser invokes the desktop app with the authorization response URL. It then renders a post-login page.

![browser logged in](../images/420/browser-loggedin.jpg?v=20240922)

The desktop app uses the authorization code in the authorization response to get OAuth tokens. The app stores tokens in memory in the main side of the app and then gets data from a <a href='serverless-api-overview.mdx'>cloud hosted OAuth-secured API</a> and renders results.
