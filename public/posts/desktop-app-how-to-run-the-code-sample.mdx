...
When login completes, the app gets OAuth tokens, stores them in memory in the main side of the app and returns itself to the foreground. It then gets data from a <a href='serverless-api-overview.mdx'>cloud hosted OAuth-secured API</a> and renders results.
