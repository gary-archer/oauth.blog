...
Previously I provided a brief <a href='desktop-apps-overview'>Desktop Code Sample Overview</a>. Next I explain how to run a more detailed desktop code sample on your local computer, which manages OAuth behaviors in a more complete way.
...
The desktop app re-uses some code from this blog’s <a href='improved-spa-code-sample-overview'>second SPA</a> and has exactly the same views:
...
When login completes, the app gets OAuth tokens, stores them in memory and returns itself to the foreground. It then gets data from a <a href='serverless-api-overview'>cloud hosted OAuth-secured API</a> and renders results.
...
- I explain some <a href='desktop-app-coding-key-points'>Desktop App Code Details</a>.
