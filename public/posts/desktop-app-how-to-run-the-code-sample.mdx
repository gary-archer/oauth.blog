...
![repo](../images/420/repo.jpg?v=20260626)

### Step 2: OAuth Configuration

The desktop app has a JSON configuration file which includes OAuth and API settings that reference AWS endpoints.
...
### Step 3: OAuth Client Registration
...
At the time of writing, AWS Cognito does not properly support loopback redirect URI behaviours from RFC 8252. It requires a base URL of *http://localhost* instead of *http://127.0.0.1*, rejects paths like */callback*, and I also had to register a set of absolute redirect URIs.
...
![child windows](../images/420/child-window.jpg)
...
### Step 5: Run a Desktop Login

The app then spins up an HTTP server to listen for a login response, then creates an authorization request URL and opens the default system browser at that URL. You can use this blog's test user account to sign in:

- User: *guestuser@example.com*
- Password: *GuestPassword1*

Once user authentication completes, the system browser returns an authorization response URL to the HTTP server, which the desktop app receives. The desktop app processes the authorization code from the response, to get a set of OAuth tokens, which it stores in memory and uses to call the <a href='serverless-api-overview.mdx'>cloud hosted OAuth-secured API</a>. The desktop app then redirects to a post-login completion page.
...
The login completion page is a simple HTML page. A real desktop app could use a more visual page or redirect to an organizational website. I uploaded the login completion page to an AWS S3 Bucket and enabled an HTTPS URL using the AWS CloudFront distribution for this blog's final SPA:
...
I used this blog's updated SPA for the views of an OAuth-secured desktop app, which runs on all of the main operating systems. The final desktop code sample will add further features to the desktop app's implementation.
