...
At the time of writing, AWS Cognito does not properly support loopback redirect URI behaviors from RFC 8252. It requires a base URL of *http://localhost* instead of *http://127.0.0.1*, rejects path like */callback* and I also had to register all possible redirect URI values.
...
The initial desktop app implements only a basic logout, that discards its tokens and returns it to the *Login Required* view. All authorization requests use the OpenID Connect *prompt=none* parameter to force a new login. In a real organization this would enable you to login as a different test user. However, AWS Cognito does not support this parameter, so this behaviour does not work as intended.
