...
In our previous post we described our <a href='spring-boot-api-oauth-integration'>Spring Boot API OAuth Integration</a>. We will now drill into some final implementation details.
...
The logging classes write to a log file and, if this blog’s <a href='log-aggregation-setup'>Log Aggregation Setup</a> is followed, logs will automatically flow to Elasticsearch:
...
The API implements this blog’s <a href='error-handling-and-supportability'>Error Handling and Supportability</a> design. By default all errors escape to the global exception handler class, whose role is to log errors and return an error response to the caller:
...
- Next we will run some <a href='api-automated-tests'>API Automated Tests</a>
