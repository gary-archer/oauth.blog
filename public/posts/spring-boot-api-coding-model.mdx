...
This blog's earlier Node.js and .NET APIs are *Non Blocking* and use an async await syntax, to prevent threads being tied up during I/O completion. The Node.js and .NET APIs use *Promises* and *Tasks* to achieve this, with only a small impact on its overall code. In Java, the same requirement exists.
