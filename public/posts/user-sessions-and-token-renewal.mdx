...
When you use iframe-based token renewal, the request can take a second or two to complete, so the user impact must be considered. If you instead use refresh tokens to renew access tokens, the time taken is much quicker, such as 100 milliseconds, and the user impact is usually negligible. 
...
Note that an SPA can use PKCE to protect its authorization code grant requests. Yet the refresh token grant for an SPA has no protection, since the SPA has nowhere secure to store a client credential. An example token refresh request is shown here:
