...
The <Link href='/posts/updated-spa-coding-key-points'>Updated SPA – Code Details</Link> explains some code complexities if you attempt to implement iframe-based access token renewal with in-memory storage. Even with careful code there are security, reliability and usability issues due to cookie issues.
