...
The app has a similar code layout to the initial desktop code sample. The *src* code represents the Electron main process and the Electron renderer process, and includes some shared code.
...
The Electron renderer process continues to use this blog's <a href='web-technology-setup.mdx'>Web Technology Setup</a>. That post explains how the frontend's entry point is an *index.html* file that references bundles. The development setup uses the rollup bundler to build modern ECMAScript bundles. The web development setup also combines a restrictive content security policy with live reloading.
...
To receive private use URI scheme notifications deterministically, the desktop app restricts itself to a single running instance:
...
The overall behaviour is similar to that of the initial desktop code sample. If the user happens to click the *Sign In* button more than once, the application continues to correctly handle re-entrancy and prevent any leaked resources.
...
The desktop code sample handles these conditions with the same code as this blog's final SPA code sample. See the <a href='reactjs-codingkeypoints.mdx'>Final SPA - Code Details</a> blog post to understand the API client logic.

### AppAuth Libraries

This blog completes this blog's desktop integration using the recommendations from RFC 8252. Doing so does not mandate use of the AppAuth libraries though. For example, if you run into a library blocking issue, you could implement the code flow messages manually in the app's *OAuthServiceImpl* class.
...
- I explain an Android OAuth Setup and run the <a href='android-setup.mdx'>Android AppAuth Code Sample</a>.
