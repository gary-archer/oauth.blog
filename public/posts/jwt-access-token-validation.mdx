...
In the access token validator object I then run the following code on every API request:
...
The API expresses its serttings in its configuration file. At the time of writing, AWS Cognito does not allow me to issue an audience claim that contains the API's identity, so I leave the value blank when using that authorization server.
...
The API provides the JOSE library with the authorization server's JWKS URI, from which the library downloads token signing public keys. The API configures this value to point to its authorization server, so that malicious tokens are always rejected. For AWS Cognito I use the following JWKS URI:
...
You can use a [JWT utility website](https://jwt.io/) to manually paste in an access token and the public JSON Web Key with which to validate its signature. If you paste in a value from the JWKS endpoint whose kid does not match that in the JWT header you get a verification failure:
