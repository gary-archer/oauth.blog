...
This blog's <a href='net-core-code-sample-overview/'>OAuth Secured .NET API</a> uses certificate trust inherited from the operating system SSL trust store.

### Trusting a Root Certificate in Java APIs

This blog's <a href='java-spring-boot-api-overview'>OAuth Secured Java API</a> uses certificate trust from the Java trust store. For example, on Ubuntu Linux this is at the following location when running Java 21:
...
For this blog's <a href='desktop-apps-overview'>OAuth Secured Desktop Apps</a> there are two processes, which determine certificate trust with different rules:
...
- I provide some design articles, starting with <a href='user-data'>User Data Management</a>.
