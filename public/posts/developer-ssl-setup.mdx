...
Later in this blog I create a <a href="cloud-domain-setup.mdx">Cloud Domain Setup</a> for a deployed domain of *authsamples.com*, which runs in AWS. I then use [AWS Certificate Manager](https://aws.amazon.com/certificate-manager/) to create AWS wildcard certificates for these subdomains, that are trusted by clients that call the SSL endpoints.

![managed certificates](../images/110/managed-certificates.jpg?v=20241231)

In real deployed environments, internet SSL certificates for APIs are typically managed by an API gateway, and internet SSL certificates for SPAs are typically managed by a content delivery network.

### Development Certificate Issuance

On a development computer, I create a SSL certificates and keys for the *authsamples-dev.com* domain and its subdomains. To do so, I use the following steps:
...
Next, I run commands to create an SSL private key and public certificate:
...
- Save the SSL key and certificate to a password protected PKCS#12 file with a *.p12* extension.
...
This blog's code samples include bash scripts that you can run to create OpenSSL certificates for local development. When SPAs or APIs start, they load the password-protected P12 file, then listen on a port. I use port 443 for SPAs and port 446 for APIs.
...
Is is therefore straightforward to run SPAs and APIs using SSL. Tools may enable you to verify the details of the client to server connection. For example, the Google Chrome browser enables you to view SSL strength from *More Tools / Developer Tools / Security*, where you can verify that the connection uses TLS 1.3.
...
My *~/trusted-ca-certs.crt* certificate bundle file has multiple entries as shown here:
...
This blog's <a href='java-spring-boot-api-overview.mdx'>OAuth Secured Java API</a> uses certificate trust from the Java trust store. For example, on Ubuntu Linux this is at the following location when running Java 25:

```markdown
/usr/lib/jvm/zulu-25-amd64/lib/security/cacerts
```
...
```bash
curl https://api.authsamples-dev.com:446/api/companies
openssl s_client -connect api.authsamples-dev.com:446
```

When required, you can override the certificate bundle file. The OpenSSL output can provide detailed output on any trust errors.

```bash
curl --cacert ~/trusted-ca-certs.crt https://api.authsamples-dev.com:446/api/companies
openssl s_client -CAfile ~/trusted-ca-certs.crt -connect api.authsamples-dev.com:446
```

### Trusting a Root Certificate in iOS and Android

Mobile SSL trust has its own complexities, which are covered in the following posts on mobile setup:
...
I explained how to upgrade development setups to use real-world HTTPS URLs for backend components. When doing so you should also learn how to configure trust so that you can view OAuth-related requests.
