...
| Extensible Claims | APIs are in full control of the claims principal and can add extra authorization values to it. |
...
This blog's Java API contains plumbing code that I write in the same way across three different technology stacks. For your own solution you may be able to meet similar requirements in simpler ways.
...
| jwksEndpoint | The trusted authorization server endpoint from which *jose4j* will download token signing public keys. |
...
I disable Spring's website behaviours that are not relevant to APIs. Instead, this blog's <a href='final-spa-overview.mdx'>Final SPA</a> uses the following backend components to manage web-specific security, so that API code is always identical for web and mobile clients.
...
The Spring authentication filter implementation runs OAuth logic to validate the access token and get claims. The filter also makes the *ClaimsPrincipal* object resolvable by business logic and controls OAuth error responses returned to clients.
...
This technique adds complexity that you may not need. However, it shows a way for API developers to evolve their own authorization values. Doing so can avoid deeper complexities, like frequently needing to change the access token payload, deal with access token versioning, and coordinate deployments of APIs and the authorization server.
