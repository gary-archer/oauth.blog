...
When getting started with OAuth secured APIs these are good defaults, but this blog's Java API focuses on some deeper requirements.
...
I disable Spring's website behaviors that are not relevant to APIs. Instead, this blog's <a href='final-spa-overview.mdx'>Final SPA</a> the following backend components manage web-specific security, so that API code is always identical for web and mobile clients.
