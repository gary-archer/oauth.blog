...
In the first sample's <a href='basicapi-codingkeypoints.mdx'>API Code Details</a> I explained that OAuth processing consists of the following two main code areas:
...
The updated API implements <a href='jwt-access-token-validation.mdx'>JWT Access Token Validation</a> in the same way as the first code sample, but now also implements this blog's <a href='api-authorization-design.mdx'>API Authorization Behaviour</a>. The API security login begins with enforcing required scopes:
...
When a token is first received, the API caches extra claims using the [Node Memory Cache](https://github.com/mpneuried/nodecache). The code sets a time-to-live that does not exceed the access token's *exp* (expiry) claim:
...
When the API receives the same access token on subsequent API requests, it immediately retrieves cached claims.
...
This mechanism can be useful in some types of mature systems, that need to authorize using user attributes from both the authorization server and business data. It avoids the need to frequently add business user attributes to the authorization server.
