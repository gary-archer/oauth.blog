...
The previous post I explained some <a href='updated-spa-coding-key-points'>SPA Code Details</a> for the second code sample. Next I summarise the main API code changes.

### API Authorization Implementation

In the first sample’s <a href='basicapi-codingkeypoints'>API Code Details</a> we explained that OAuth processing consists of the following two main code areas:
...
The updated API implements <a href='jwt-access-token-validation'>JWT Access Token Validation</a> in the same way as the first code sample, but now also implements this blog's <a href='api-authorization-design'>API Authorization Behaviour</a>. This starts with enforcing its required scope:
...
- I port the second code sample to another provider, in the <a href='azure-active-directory-setup'>Microsoft Entra ID SPA and API Setup</a>.
