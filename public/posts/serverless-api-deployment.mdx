...
Previously we provided a <a href='serverless-api-overview'>Serverless API Overview</a> and explained how to run its operations locally. Next we will explain how this API is deployed to the AWS cloud, to run as lambda functions.
...
The Serverless API will provide data to SPA, mobile and desktop clients. Although the final <a href='final-spa-overview'>Single Page Application</a> uses secure cookies, the API will only need to deal with JWT access tokens, for all types of clients.
...
- Create an AWS upload account according to the <a href='cloud-domain-setup'>Cloud Domain Setup</a>
...
The main hosting domain of *authsamples.com* was created in the earlier <a href='cloud-domain-setup'>Cloud Domain Setup</a>. An API custom domain must also be pre-created in the AWS API Gateway. This points to the wildcard certificate generated earlier, and is linked to the deployment stage under *API Mappings*:
...
Log entries can be viewed in the AWS console, but CloudWatch is only used for immediate logging. In a real company setup, logs should be aggregated to a more powerful log analysis system with <a href='api-technical-support-analysis'>good querying capabilities</a>.
...
- Next we will explain some <a href='serverless-api-coding-model'>Serverless API Code Details</a>
