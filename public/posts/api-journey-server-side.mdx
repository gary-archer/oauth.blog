...
The API validates a JWT access token on every request, after which the API trusts the token claims. The API adds identity details to logs and then performs its business authorization, using *scopes* and *claims* from the access token's payload.
...
### APIs Returns Friendly Errors to Clients
...
### APIs Audit Requests for Sensitive Data

Once you take control over API logging, it should be straightforward to implement auditing of API requests. This is just a different type of log, which records identities and the level of access. In particular, it outputs the scopes and the most important business claims. This blog's final APIs write audit logs with the following fields:

```json
{
  "type": "audit",
  "id": "d6edef10-0ae9-44fe-91b6-95e68d2b9d3b",
  "utcTime": "2025-10-11T18:21:39.991Z",
  "apiName": "FinalApi",
  "operationName": "getCompanyTransactions",
  "hostName": "UBUNTU",
  "method": "GET",
  ...
  "userId": "a6b404b1-98af-41a2-8e7f-e4061dc0bf86",
  "statusCode": 200,
  "sessionId": "51bea312-b713-46eb-a9f4-8af2c4328836",
...
You can think of technical support logs and audit logs as separate schemas that different audiences can be granted access to. The [Log Aggregation Setup](log-aggregation-setup.mdx) post explains how to deploy Elasticsearch and Kibana so that:

- A support user can ask questions of technical support logs.
- A security user can ask questions of audit logs.

A real-world deployment should include aggregated logs for both APIs and the authorization server. The authorization server is a specialist API that should provide its own audit logs that record security events, to provide insights into usage and potential threats.
