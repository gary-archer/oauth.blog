...
Previously I summarised this blog's <a href='api-platform-design.mdx'>API Platform Overview</a> to express some essential high-level ingredients. Next I summarise some end-to-end technical behaviours for OAuth-secured APIs, which I implemnt in this blog's final API code samples, in multiple technology stacks:
...
export class CompanyController extends BaseHttpController {
    ...
    public async getCompanyTransactions(@requestParam('id') id: string): Promise<CompanyTransactions> {
        return this.service.getCompanyTransactions(id);
    }
}
```
...
export class CompanyService {
    ...
    private readonly claims: SampleClaimsPrincipal;
