...
The API and web static content hosting use SSL so you must create development certificates. Ensure that OpenSSL 3+ is installed and then run the following script:
...
Next run the following script, to build the code and run the API over SSL on port 443. The default browser then runs at *https://www.authsamples-dev.com/spa*. The SPA redirects to Microsoft online and you can sign in with one of the test user accounts:
