...
The goal of the Serverless API is to implement this blog's <a href='api-platform-design.mdx'>API Architecture Goals</a> and also use low cost cloud hosting. Readers of this blog can then easily run a <a href='quick-start.mdx'>Frontend Code Sample</a> on their local computer, to understand the OAuth application lifecycle. That lifecycle includes tricky aspects like handling token expiry events when sending concurrent API requests.

### Serverless API Overview

The Serverless API behaves equivalently to the earlier <a href='api-architecture-node.mdx'>Node.js Express API</a>. The API has a couple of simple endpoints that return mock data for a money-based theme of investments. The API aims to be visual and serves this blog's apps, such as the <a href='how-to-run-the-react-js-spa.mdx'>Final SPA</a> below. If required, you can run both the API and a client, to ensure that the API is client-focused and to view API logs that the client generates.

![spa and api](../images/620/spa-and-api.jpg?v=20240915)

### API URLs

During development the API runs as a standalone component. You can use either mocha tests or a load test as the API client. To call the API, tests must get user-level access tokens. The API accepts mock access tokens when you point it to a mock authorization server:
...
First, update your computer's hosts file to include the API and authorization server domains for local development. If you run the API and SPA together, also include the web domain and local backend-for-frontend domain:
...
To run API tests, or the full SPA and API solution, you need Docker and Docker Compose, so install a local engine like [Docker Desktop](https:/www/.docker.com/products/docker-desktop).
...
In AWS it is common to use a [Lambda Authorizer](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-jwt-authorizer.html) to implement security only at the internet entry point. Secure values such as claims are then simply forwarded to lambdas in plain HTTP headers. This is not the most secure design. APIs also lose track of the incoming access token, so the security does not scale correctly, since a lambda cannot call an upstream API with a correct access token. To resolve these issues, the serverless API code sample implements OAuth security within the lambda function's code.
...
The API implements its security according to the following two blog posts, using a JOSE library and some custom claims handling:
