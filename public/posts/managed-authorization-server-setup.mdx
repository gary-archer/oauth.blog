...
The *claimsToSuppress* loigc removes personal data from ID tokens, since my code samples instead get personal data from the Cognito user info endpoint. The *claimsToAddOrOverride* section customises access tokens. Claims issued depend on the client's scope, to enable different clients to receive distinct claims when required. Custom claims can help to lock down access tokens and reduce their privileges. This blog's use of claims is further explained in the <a href='api-authorization-design.mdx'>API Authorization Behaviour</a> post.
