...
Previously I explained this blog's <a href='web-technology-setup'>Web Technology Setup</a> for modern frontend development. Next I show how developers can visualize OAuth-related request and response messages.
...
First, edit the API configuration for this blog’s <a href='basicspa-execution'>initial SPA and API code sample</a> and set *useProxy=true*:
...
In later posts I provide an <a href='net-core-code-sample-overview'>OAuth Secured .NET API</a>. Outbound calls use a custom *HttpClientHandler* to support HTTP proxy integration:
...
In later posts I provide an <a href='java-spring-boot-api-overview'>OAuth Secured Java API</a>. You can activate the proxy with standard system properties at startup, based on the API’s configuration settings. For HTTP URLs the properties use an *http* prefix, and for HTTPS URLs they use an *https* prefix:
...
In later posts I use [Electron](https://electronjs.org/) to develop <a href='desktop-apps-overview'>OAuth-Secured Cross Platform Desktop Apps</a>, where there are two code areas:
...
- I explain how to manage <a href='developer-ssl-setup'>SSL Trust Configuration</a> during development.
