...
Previously I explained this blog's <Link href='/posts/web-technology-setup'>Web Technology Setup</Link> for modern frontend development. Next I show how developers can visualize OAuth-related request and response messages.
...
First, edit the API configuration for this blog’s <Link href='/posts/basicspa-execution'>initial SPA and API code sample</Link> and set *useProxy=true*:
...
In later posts I provide an <Link href='/posts/net-core-code-sample-overview'>OAuth Secured .NET API</Link>. Outbound calls use a custom *HttpClientHandler* to support HTTP proxy integration:
...
In later posts I provide an <Link href='/posts/java-spring-boot-api-overview'>OAuth Secured Java API</Link>. You can activate the proxy with standard system properties at startup, based on the API’s configuration settings. For HTTP URLs the properties use an *http* prefix, and for HTTPS URLs they use an *https* prefix:
...
In later posts I use [Electron](https://electronjs.org/) to develop <Link href='/posts/desktop-apps-overview'>OAuth-Secured Cross Platform Desktop Apps</Link>, where there are two code areas:
...
- I explain how to manage <Link href='/posts/developer-ssl-setup'>SSL Trust Configuration</Link> during development.
