...
![mitm trace](../images/100/mitm-trace.jpg?v=20260706)

Proxy tools typically do not capture *localhost* requests. Although you may be able to get a capture working, the most productive option is to use HTTPS URLs with real-world domain names. I use HTTPS for all of this blog's code samples.
...
Next, I perform any environment specific configuration, explained in the following sections. I then debug HTTPS messages for the components of interest. Finally, when I finish debugging, I run the *stop.sh* script to free resources.
...
The tricky part of using an HTTP proxy is knowing how to configure it across different tools and technologies, which can include all of these clients:
...
In Node.js, this blog's APIs use the proxy built into the [undici fetch library](https://github.com/nodejs/undici), and set the proxy URL that corresponds to the scheme of the target URL. The *undici* library adds a *dispatcher* property to fetch options that activates the proxy. When connecting to HTTPS URLs I use the following utility class:
...
The .NET code then wires up the custom handler to the *HttpClient* class that sends HTTPS requests:
...
I commonly use the *curl* and *openssl* tools to make HTTPS requests, on Windows, macOS and Linux operating systems. These command line tools look for one or both of the below environment variables, depending on the scheme of the target URL:

| Environment Variable | Used To |
| -------------------- | ------- |
| http_proxy | Call HTTP URLs |
| https_proxy | Call HTTPS URLs |

To use curl to connect to an HTTPS URL I use commands of this form to capture traffic in an HTTP proxy:

```bash
export https_proxy=http://127.0.0.1:8888
curl -H "Authorization Bearer xxx" https://api.authsamples-dev.com:446/api/companies
```
...
Use of HTTP proxy tools for mobile apps is more intricate, involving quirks of both simulators and devices. I explain the techniques that I use in the following posts:
...
![proxy breakpoint](../images/100/proxy-breakpoint.jpg?v=20260706)

This type of technique can be used to verify that your OAuth client processes OAuth *error* and *error_description* fields correctly. This blog's frontend applications handle OAuth errors and render details in a way that helps to enable fast problem resolution:

![breakpoint error result](../images/100/breakpoint-error-result.jpg?v=20251230)

### Where Are We?

I showed how to use an HTTP proxy tool to capture HTTPS messages from clients and APIs, regardless of their technology. Doing so improves troubleshooting options during development and also promotes a solid understanding of the OAuth standards-based messages.
