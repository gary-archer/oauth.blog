...
The API only uses a handful of third-party libraries, declared in its *package.json* file. The most interesting dependency is the [jose security library](https://github.com/panva/jose) that I use to validate JWT access tokens:
...
In development mode, the API loads OpenSSL issued certificates and listens at *https://api.authsamples-dev.com:446*.
...
A mainstream way to run the API's business logic for an API request is to run a controller object, which then calls other testable business objects. In this blog's early API code samples I manually wire up an *ApiController* object with its dependencies.
...
I then inject the claims principal into business logic classes:
...
The API could easily use received claims to reject unauthorized requests and throw a forbidden error:
