...
In frontend technology stacks, a view binds to *state* in its view model. The view model can also contain other *properties*. Both types of data can originate from APIs. The view model can create other values from the raw data (the model) that APIs return. The overall process is the Model View View Model (MVVM) pattern.
...
Other technology stacks, like React, might not use view models explicitly, but you can follow the same pattern, where views receive *props* that contain a view model and the view model contains bindable state.
