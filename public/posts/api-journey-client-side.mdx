...
When OAuth clients start they typically load their configuration settings into an object. An app that acts as an OAuth public client might use settings similar to those shown below:
...
In frontend technology stacks, a view binds to *state* in its view model. The view model can also contain additional non-bindable *properties*. Both types of data can originate from APIs. The view model can create other values from the raw data (the model) that APIs return. The overall process is the [Model–view–viewmodel (MVVM)](https://en.wikipedia.org/wiki/Model-view-viewmodel) pattern.

The following example shows a View Model object for Kotlin on Android:
...
class MyViewModel(
    private val fetchClient: FetchClient
     ...
     var myState: MutableState<MyApiData?> = mutableStateOf(null)
     var myProperty: String
}
```

The following example shows a View Model object for SwiftUI on iOS:

```swift
class MyViewModel: ObservableObject {

    @Published var myState: MyApiData?
    var myProperty: String
    private let fetchClient: FetchClient

    init(fetchClient: FetchClient) {
        self.fetchClient = fetchClient
    }
}
```

When creating a view, you pass the view model as an input property. The view does not know about the internal properties of the view model, like a fetch client that calls APIs.

Other technology stacks, like React, might not use view models explicitly, but you can follow the same pattern, where views receive *props* that contain a view model and the view model contains bindable API state.
...
OAuth secured APIs can issue security challenges to frontend applications. These can occur in response to concurrent API requests:
...
To enable security control with short-lived, least-privilege access tokens, frontend applications should initiate all API requests for sensitive data, using fetch requests. For web applications, This means the web host never initiates calls to APIs.
...
Invoking these URLs runs views which in turn initiates API requests. APIs return 401 responses if an access token expires or if the user needs to re-authenticate. In such cases this blog's frontends always run the requested view once any OAuth work is complete, so that bookmarking works correctly for users.
...
The *API Session ID* shown in frontends is the OAuth *delegation_id* from the ID token. APIs receive the same value in access tokens. The *delegation_id* remains the same throughout the authenticated user's session and this blog's final APIs include the value in their API logs. Testers can therefore filter API logs to find entries they have generated. See the <a href='api-technical-support-analysis.mdx'>Technical Support Analysis</a> post for some example queries that use the logged session ID.

### Client Responsibilities are Separated

To provide the behaviours that I articulate, this blog's clients require some plumbing code. A folder structure classifies each class as *view* related or *plumbing* related. In real clients you might externalise some of the plumbing to one or more shared libraries. The goal in a real organization would be to use clean, scalable code for the valuable views and view models.
