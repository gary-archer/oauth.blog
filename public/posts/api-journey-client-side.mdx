...
To enable the reliable use of short-lived, least-privilege access tokens, the app itself should use fetch requests to initiate all API requests for sensitive data. For web applications, that should be code that runs in the browser. This blog's <a href='web-technology-setup.mdx'>Web Technology Setup</a> meets that requirement by ensuring that the web host never comes into contact with sensitive data from APIs.
