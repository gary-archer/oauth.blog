...
After user authentication the app receives an authorization response that contains an authorization code. An intermediate web page manages the claimed HTTPS scheme redirect URI to ensure that the app reliably receives the response.

![authorization response](../images/510/authorization-response.jpg?v=20240912)

The app uses two intermediate web pages, hosted at the following URLs:
...
If the user leaves the app at the intermediate page for a couple of minutes and then clicks *Return to the App*, the authorization code could time out, leading to a user error. The user can always retry and recover though.
...
Note that a mobile app is a public client and by default it does not usually protect the refresh token with a client credential. With additional work you might be able to further protect this request, using a technical behavior like [mobile attestation](https://approov.io/mobile-app-security/rasp/app-attestation/).
