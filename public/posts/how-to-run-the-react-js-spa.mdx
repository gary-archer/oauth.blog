...
You can run some scripts to test a release deployment. The *build.sh* script produces JavaScript release bundles. The *deploy.sh* script runs a Docker container that uses Express to serve minimized static content. The *teardown.sh* script stops the Docker container.
...
After the *deploy.sh* script, browse to *https://www.authsamples-dev.com/spa/* and test a release deployment of the SPA. Release builds use web resource optimizations, like minification, compression and browser caching, but otherwise execute the same as development builds.
