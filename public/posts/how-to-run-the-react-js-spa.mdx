...
You will then be prompted to sign in, and can do so with the following test credential:
...
In earlier samples, the SPA performed its own JavaScript code flow. This work is now done by remote token handler components, and the SPA configuration is therefore simplified, primarily to a backend for frontend base URL:
