...
Open a terminal and run the following command from the root folder, to do some basic setup such as installing dependencies:

```bash
./build.sh
```

### Step 5: Configure SSL Trust

The build script downloads some OpenSSL issued development certificates. You should ensure that your browser trusts the root certificate at the below location, according to the <a href='developer-ssl-setup.mdx'>SSL Trust Configuration</a> post.

```markdown
certs/authsamples-dev.ca.crt
```

### Step 6: Execute the Run Script

Next run the following command from the original terminal window, to serve web content using the [Webpack development server](https://webpack.js.org/configuration/dev-server/) and open the default browser at *https://www.authsamples-dev.com*:

```bash
./run.sh
```

The frontend runs in watch mode, so that you follow a pure SPA development model. You can update the React code and quickly see changes in the browser:
...
### Step 9: View Updated SPA Configuration
...
### Step 10: Understand Web Security Headers
...
### Step 11: Run a Release Build

You can run some scripts to test a release deployment and run the SPA at the same local computer URL. The *build.sh* script produces JavaScript release bundles. The *deploy.sh* script runs an Express Docker container that serves static content in the same way as the webbpack development server. The *teardown.sh* script stops the Docker container.
