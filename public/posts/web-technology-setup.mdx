...
The code setup supports bundle splitting to reduce the initial download size. You can then write the following type of syntax to trigger the download of additional incremental JavaScript bundles as the user navigates to new areas of the application:

```typescript
const module = await import('myLazyLoadedModule');
const myObject = new module.MyClass();
const result = myObject.myMethod();
```

The use of the *chunkFilename* setting means that webpack generates random bundle names like *506.bundle.js* for lazy loaded code. These bundles get built to the *dist* folder but are excluded from the *index.html* file.

You can also use the [SplitChunksPlugin](https://webpack.js.org/plugins/split-chunks-plugin/) to split the vendor bundle into multiple chunks. The following example shows the approach. You could use this technique to build a separate vendor bundle that includes dependencies only used by lazy loaded code.
