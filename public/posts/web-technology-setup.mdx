...
This blog's APIs use Node.js 24 or later, with the following *tsconfig.json* configuration. You use some of these settings when you import modules and other settings when you build code:
...
There is a similar configuration file at *spa/.vcscode/launch.json* to launch the SPA:
...
When bundle files download in debug mode, their last line points to a source map file that the browser also downloads:
