...
The *tsconfig.json* file for the SPA is a little different to that for the Node.js API. The *lib: DOM* option brings in browser type definitions and *ES2024* enables the most modern coding syntax:
...
Strong typing during coding allows the environment to make more checks and simplifies refactoring. I use ECMAScript modules so that the compiled code will remain modern and closest to the original TypeScript syntax.

### Writing Code: Styling

For this blog I keep styling simple, since some CSS bundling solutions add a lot of technical overhead. I mostly use the [Bootstrap](https://getbootstrap.com/docs/5.3/getting-started/introduction/) library's built-in styles, in particular to enable simple grid based layouts.
...
There are various web bundlers, and I use the [webpack](https://webpack.js.org/) tool. It requires a configuration object that you can create in TypeScript. The options can seem a little cryptic at first. Mostly though, you just need to specify the SPA's entry point source file, then provide an output folder for compiled bundles. The following example uses an entry point of *app.ts* and writes bundles to a *dist* folder.
...
You could adapt this blog's SPAa to use an alternative web bundler if you prefer:

- [Rollup](https://rollupjs.org/) requires a fairly complex configuration, like webpack, but can produce the cleanest compiled code for production releases. However, I find that its development options lack some important features.
- [Vite](https://vite.dev/) provides a simpler configuration and some developer conveniences. However, it seems not designed to meet my main requirement to run with the same technical mechanisms during development and production.

All in all I currently prefer webpack, since it is a mature, open source solution that seems to have a strong consumer-focus when you run into deeper requirements.
...
| [module](https://www.typescriptlang.org/tsconfig#module) | ES2015 means bundled code uses the ECMAScript module format, which browsers have supported for years. |
...
One current limitation is that webpack ECMAScript module syntax is not as clean as that produced by the rollup bundler. In future I would like code that runs in the browser to be as close as possible to my TypeScript code.
...
This blog's SPAs use a code setup that supports bundle splitting to reduce the initial download size. You could add a new class and dynamically use it from a view, with code similar to the following. When you navigate to that view, the browser tools would show an additional bundle download.
...
Webpack bundles are referred to as *chunks*, where you express *initial chunks* in the *index.html* file. The build also produces *non-initial chunks* and writes them to the output folder. Initial chunks contain references to non-initial chunks.

The [SplitChunksPlugin](https://webpack.js.org/plugins/split-chunks-plugin/) enables you to partition both your own code, and the *node_modules* folder, into chunks. The following example creates an extra chunk for React libraries and an extra chunk for shared components. Downloads of non-initial chunks occur on the first dynamic import that uses one of the chunk's exported classes or functions.
...
I then render stack traces as raw strings in debug builds:

![exception](../images/90/frontend-exception-display.jpg)

I can then use a [retrace tool](https://github.com/fgnass/retrace) to convert a raw stack trace back to its original unbundled lines of code, with logic similar to the following:

```javascript
import fs from 'fs';
import retrace from 'retrace';

const sourceMap = fs.readFileSync('./mymodule.js.map', 'utf8');
retrace.resolve = (url) => {
  return sourceMap;
};

const rawStack = fs.readFileSync('./stack.txt', 'utf8').replace(/\\n/g,'\n');
console.log(rawStack);

const originalCodeStack = await retrace.map(rawStack);
console.log(originalCodeStack);
```

In bundled applications I save source map files for each application release. I also rehearse exceptions during development, to verify that stack trace lookups will work as expected if I need to diagnose production stack traces later.
...
- The <a href='final-spa-overview.mdx'>Final SPA Overview</a> explains updates to the React framework and to use a strong content security policy.
...
Of course, in a real SPA you are likely to have additional requirements, like scalable ways to manage images and CSS. You may also prefer to do a little more work in the web host. You should be able to adapt this blog's web technology setup to add extra behaviors that meet your own requirements.
