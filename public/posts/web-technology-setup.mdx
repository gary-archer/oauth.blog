...
### OAuth Web Architecture

Before OAuth and APIs, web applications required a web backend to secure requests. Web developers built backend endpoints secured by HttpOnly cookies. The web backend required a runtime and cookie issuing libraries. The runtime imposed deployment limitations and sometimes imposed unwelcome usability problems, like abrupt server issued login redirects that frontend developers could not control.

In an OAuth web architecture, APIs manage data security instead of the web static content host. Content delivery networks can optimize web performance. Web developers have fewer responsibilities and can use up to date technology to build a modern frontend experience, with a faster time to market than older solutions.

To fully realise the benefits you first need the type of well designed web technology setup that this post explains. Web developers can then operate like mobile developers, with a frontend-only focus and full usability control. To get sensitive data, the frontend sends access tokens (or HttpOnly cookies that reference them) directly to internet API URLs. 
...
After the web build, the SPA's HTML, CSS and JavaScript bundles can be served from any HTTP server, so deployment is portable. For example, Node.js Express can serve static content using the following minimal code:
...
As is standard for SPA development, this blog's SPAs use a development static content host. I use the webpack development server, which uses Node.js Express under the hood. To serve static content locally using SSL and a custom domain name I use the following configuration:
...
Although I get some developer conveniences like hot reloading when I make frontend code changes, the main behavior I want from a development web static content host is to serve web assets in the same way as for production users. The only difference during development is that I use a non-optimized build with debugging support.
...
In debug builds I intentionally render SPA stack trace data in a raw format. In production apps, I could use a *Send Home* feature to receive this data:
...
Once you have a productive web pipeline, you are well placed to productionize and apply finishing touches to the web technology. The following blog posts explore this blog's finishing touches, all of which build upon the web technology setup:

- The <a href='spa-back-end-for-front-end.mdx'>Backend for Frontend Design</a> explains security hardening for SPAs.
- The <a href='final-spa-overview.mdx'>Final SPA Overview</a> explains an update to the React framework.
- The <a href='api-journey-client-side.mdx'>API Journey Client Side</a> explains more about API sessions in clients.
...
Of course, in a real SPA you are likely to have additional requirements, like greater use of images and CSS. You may also prefer to do a little more work in the web host. You should be able to adapt this blog's web technology setup to implement such requirements.
