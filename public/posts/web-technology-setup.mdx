...
I also inform the compiler that the project's own source files use ECMAScript modules. To do so, I specify *type=module* in the *package.json* file. For Node.js (non browser) code that use *type=module*, add a *.js* suffix to imports. During coding this is a TypeScript file, though it becomes JavaScript when executed:
...
You could adapt this blog's SPAs to use an alternative web bundler. I currently prefer webpack over other bundlers, since it is a mature, open source solution that I find meets my deeper requirements, for both production and development, to avoid blocking issues.
...
Webpack bundles are referred to as *chunks*, where you express *initial chunks* in the *index.html* file. The build also produces *non-initial chunks* and writes them to the output folder, with names like *506.bundle.js*. Initial chunks contain references to non-initial chunks.
...
Source map stack trace lookup also works for minified release builds, to provide a mechanism to help resolve production errors. I save source map files for each application release. Production builds can use a feature to send a stack trace rather than display it to the user.
