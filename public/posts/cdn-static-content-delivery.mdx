...
In total I use the following domains for web content, each of which has an S3 buckets and a CloudFront distributions:

| Domain | Usage |
| ------ | ----- |
| www.authsamples.com | Serves web content for an online SPA that connects to Serverless APIs. |
| www.authsamples-k8s.com | Serves web content for an online SPA that connects to Kubernetes APIs. |
| mobile.authsamples.com | Serves assets used of native mobile logins and mobile deep linking. |

This post covers the details for the first domain, and the steps for other domains are equivalent. First though I briefly summarise some build related steps that help to enable a good deployment architecture.
...
I instead hard code the production configuration, to avoid the need for configuration downloads, and because JSON files do not have subresource integrity protection. When getting configuration, the SPA first checks its web origin and uses hard coded configuration when it runs in a production domain:
...
Once the upload completes a number of files exist in the S3 bucket, where the SPA files are located in an *spa* folder. Other micro frontends could be hosted in parallel folders at a later time.
...
If you try to browse to the SPA you get an access denied error, since only AWS CloudFront is granted access:

![direct access denied](../images/610/direct-access-denied.jpg)

### Web SSL Certificates

During the <a href='/posts/cloud-domain-setup.mdx'>Cloud Domain Setup</a> post I explained how I provisioned SSL certificates for the *us-east-1* (North Virginia) region. The wildcard certificate for **.authsamples.com* is used to serve the SPA's content over HTTPS:
...
The code running within a CDN only manages web concerns. I avoid activities such as calling APIs, since CDNs may impose short code timeouts and small HTTP message sizes. In the AWS console, the behaviour settings for the final SPA are configured as follows:

![cloudfront functions](../images/610/cloudfront-functions.jpg)

### Upgrading the SPA

When you deploy updated HTML assets they are not automatically deployed to all of the global edge locations. To prevent users from receiving old content, force edge locations to pull updated content from the S3. The *deploy.sh* script does so by executing a CloudFront invalidation for all web resources. In larger deployments you could instead only invalidate modified resources.
