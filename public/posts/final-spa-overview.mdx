...
This blog's initial SPAs used vanilla TypeScript, Bootstrap for CSS and webpack as a bundler and development web server. The final SPA uses React, with [Rollup](https://rollupjs.org/) as a bundler that produces the best ECMAScript code output to run in the browser. These changes may give the final SPA greater developer appeal, but they do not change the <a href='web-technology-setup.mdx'>Web Technology Setup</a>.
...
<div className='smallimage'>
    <img src='../images/220/code-layout.jpg?v=20251230' alt='code layout' />
</div>
...
The SPA's code now uses React as a web framework, to enable richer coding options in areas like view development, navigation and code splitting. The SPA renders a tree of views which leads to concurrent API requests. The SPA must synchronise the triggering of both token renewal and user re-authentication when tokens expire.
...
| Secure SPA Access | The application reload, followed by API requests to get sensitive data specific to the user. |
...
| Download Sizes | As the SPA's code grows I would aim for medium-sized downloads using <a href='web-technology-setup.mdx#building-code-bundle-splitting'>bundle splitting techniques</a> and micro-frontends. |
