...
Previously I explained the <a href='spa-back-end-for-front-end'>Backend for Frontend</a> setup for the final SPA, where remote API components issue cookies on behalf of the SPA. Next I summarise the main frontend behaviours of the completed SPA, after which I show how to run it locally.

### Key Features

The final SPA improves on the previous <a href='improved-spa-code-sample-overview'>Updated SPA</a>, to meet some key requirements. It is difficult to satisfy all of these at the same time:
...
I deploy the API code sample to AWS as <a href='serverless-api-overview'>Serverless Lambda Functions</a> that the AWS API Gateway exposes to the internet. I find this API technology a little suboptimal in some areas, including performance, but it provides low cost backend hosting for this blog's demo APIs.

### Online Deployment

Any reader can run the AWS deployed version of the SPA from this blog’s <a href='quick-start'>Cloud Samples Quick Start</a> page. You can sign in to the SPA with a test user account, then test OAuth lifecycle events via the demo app’s buttons:
...
- I explain <a href='how-to-run-the-react-js-spa'>How to Run the Final SPA</a>.
