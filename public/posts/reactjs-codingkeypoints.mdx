...
During development I continue to use the webpack development server. For the final SPA I use additional settings to use a restrictive content security policy during development. I also run middleware to handle invalid paths outside of the React root location:
...
The project also contains an Express web host that you can find in the *deployments/docker/webhost* folder. You can use the Express host to test delivery of static content release builds to the browser. The Docker web host uses compression and writes response headers to enable browser caching. I say more about release behaviors at the end of this blog post.
...
I use the React framework's hook-based syntax to develop the SPA's views. Views call *use* on the view model and the view model call's React's *useState* hook. This mechanism externalizes API concerns from views in the same way as mobile technology stacks.
...
private async callApiWithCredential(
    ...
    const headers: any = {
        ...
        'correlation-id': Guid.create().toString(),
    };
