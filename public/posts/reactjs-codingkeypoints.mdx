...
There is one remaining post for this blog's final SPA. It is part of the cloud theme and covers <a href='cdn-static-content-delivery.mdx'>Web Content Delivery</a> using a cloud platform. The SPA's static content gets uploaded to a content delivery network which deploys it to hundreds of global locations for globally equal performance. The SPA's deployment is also portable and runs the same anywhere, including a development computer.
