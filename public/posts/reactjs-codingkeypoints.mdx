...
The final SPA uses Express as a development web server. The server exposes a utility reload endpoint that Rollup watch mode calls when bundles are rebuilt after code changes. The reload endpoint sends a notification to the browser client, which reloads itself.
...
The development web server returns security headers during the download of web resources. Headers include a highly restrictive content security policy.
...
The final SPA uses the Rollup bundler to produce clean ECMAScript bundles whose code runs in the browser. A partial configuration is shown below, and is similar to the webpack configuration from this blog's earlier SPAs. Getting your preferred results typically requires the use of built-in or custom plugins, where you can customize behaviours. During the overall build, an *esbuild* plugin translates TypeScript code to JavaScript.
...
After login the SPA uses *HttpOnly SameSite=strict Secure* cookies to call APIs. The SPA must use the *credentials: include* fetch option to instruct the browser to send cookies to its backend for frontend. The SPA sends a custom header to enforce CORS preflight requests, as a CSRF prevention best practice. The SPA also sends some a correlation ID for logging and tracing purposes.
