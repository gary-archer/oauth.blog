...
Views get recreated frequently, whereas I manage state returned from APIs within view model classes that get created only once. I manage this by storing child view model references within the root *AppViewModel*:
...
I use the React framework's hook-based syntax to develop the SPA's views. Views get API data from their view model, to externalize API concerns from views in the same way as in mobile technology stacks.
...
After login the SPA uses *HttpOnly SameSite=strict Secure* cookies to call APIs. The SPA must use the *credentials: include* fetch option to instruct the browser to send cookies to its backend for frontend. The SPA sends a custom header to enforce CORS preflight requests, as a CSRF prevention best practice. The SPA also sends a correlation ID for logging and tracing purposes.
