...
Cloud platforms provide infrastructure as a service that makes people more productive. You can provision remote hardware on demand, for storage, compute and networking. In particular the networking provides HTTPS URLs for APIs, web static content and other endpoints. You then build and upload application binaries to provide working applications.

### Productive Development

In particular, cloud platforms can empower developers to learn more and to build powerful systems. For this blog I purchased an AWS development domain at *authsamples.com* at low cost, which I use to host applications. Developers no longer need to rely upon IT departments to enable addressable applications.
...
In an OAuth architecture, you need at least the following capabilities on the API side of the architecture, as summarised in the previous post on <a href='api-platform-design.mdx'>API Architecture Goals</a>:

- API code runs without limitations, in the same way as on a development computer.
- An authorization server provides up to date user authentication and token issuance capabilities.
...
Supporting components for APIs is an area where cloud native tends to perform better than serverless deployments. You host supporting components right next to your APIs and then need to do more deployment work to keep them up to date. Conversely, the web side of the architecture is only static content so does not need supporting components.

### Availability and Performance

Both types of deployment use design patterns to keep your APIs available and performant:
