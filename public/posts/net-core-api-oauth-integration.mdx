...
To get visibility into the Microsoft code behaviour, you can configure a log level for the *Microsoft.AspNetCore.Authentication.JwtBearer* namespace. When getting started with OAuth secured APIs these are good defaults, but this blog's .NET API focuses on some deeper requirements.
...
The API contains quite a bit of plumbing code, to make the API code feel the same across technology stacks. For your own solution you may be able to meet similar requirements in simpler ways.
...
The API receives an access token with the following payload to restrict the client's API access. The scope is limited to *investments* data. The user's business identity is a custom claim of *manager_id*, for a party who manages investments. The authorization server also issues a custom claim for *role* to the access token:
...
The API processes the JWT access token to receive the main claimns, The API then does some more advanced work to look up additional values that I want to treat like claims in the API code. The goal is to set up the API's business logic with a useful claims principal.
...
| 404 | The API's claims do not allow access to a resource so the API returns a *Not Found for User* error. |
...
After the API validates a JWT, it writes the received subject claim, most commonly a UUID, to a *userId* field in the request's API log entry. This can potentially help to track users in benign ways, for technical support purposes. Doing so requires care, as discussed in the <a href='effective-api-logging.mdx'>Effective API Logging</a> post.

### Where Are We?

I implemented .NET API Security in a requirements first manner, to enable behaviour that is likely to be important for any OAuth secured API, regardless of technology.
