...
When getting started with OAuth secured APIs these are good defaults, but this blog's .NET API focuses on some deeper requirements.
...
The *AccessTokenValidator* object wraps calls to the *jose-jwt* library and also makes some additional checks:
