...
| jwksEndpoint | The trusted authorization server endpoint from which *jose-jwt* will download token signing public keys. |
...
The implementation's main logic calls an *OAuthFilter* object, which validates the JWT access token and produces a *ClaimsPrincipal* object that the API code uses later for authorization. The handler also enforces required scopes and enables business logic classes to resolve the *ClaimsPrincipal*.
...
The *AccessTokenValidator* object gets the token signing public key that matches the *kid* field in the received JWT access token's header. Next, the *jose-jwt* library performs cryptographic validation of the access token's signature. The code then checks for expected claims and adds some access token details to logs.
...
The *jose.jwt* library does not manage JWKS downloads, caching of JWKS keys, or checking protocol claims. These tasks are not complex but make my API code less elegant. I needed to write additional plumbing classes, including a *JsonWebKeyResolver* and a *JwksCache*.
