...
Previously I summarised this blog's <a href='native-architecture-goals.mdx'>Native Architecture Goals</a>. I cover secure mobile apps in later posts. First though, I provide a couple of OAuth and OpenID Connect secured desktop code samples. They connect to <a href='cloud-hosting.mdx'>HTTPS endpoints hosted in AWS</a>, so that you only need to run the desktop app's code to get a fully integrated solution.

### Code Sample Goal

The initial desktop objective is to get an OAuth application lifecycle working for an Electron desktop app. The code sample includes a developer setup and some special handling to authenticate users on a separate browser window. The app then calls APIs with access tokens and handles expiry events.
...
| Security Library Integration | A security library helps to implement the OAuth application lifecycle and APIs calls with access tokens. |
...
I use [Electron](https://www.electronjs.org/) as a JavaScript based technology stack, where you develop desktop frontends with web technologies. I re-used the vanilla TypeScript views from this blog's <a href='improved-spa-code-sample-overview.mdx'>Updated SPA</a>:
...
A web client runs in a browser and has an addressable URL on which to receive login responses. Native apps need a different mechanism, and the [RFC 8252](https://datatracker.ietf.org/doc/rfc8252) standard explains the three options that you can use. Only the first two options work for a desktop app, and I use the loopback option for this blog's initial desktop app. The final desktop app instead uses a private use URI scheme. Later, this blog's mobile apps use claimed HTTPS schemes.

| Option | Example Redirect URI | Usable By |
| ------ | -------------------- | --------- |
| Loopback Interface | *http://127.0.0.1:8000/callback* | Desktop apps |
| Private Use URI Scheme | *x-authsamples-myapp:/callback* | Mobile or desktop apps |
...
To update the app, edit the details in the *flow.ts* module. Also, in the *app.ts* module, use *https://login.authsamples.com/oauth2/userInfo* as the userInfo URL and then replace the *updateUI* function with the following code:
