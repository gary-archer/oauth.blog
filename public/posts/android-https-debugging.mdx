...
Previously I ran an initial Android setup using the <a href='android-setup.mdx'>AppAuth Android Code Sample</a>. Next I explain a way to view HTTPS mobile traffic from Android emulators and devices on a development computer, to provide a productive troubleshooting setup.

### Proxying Solutions

On mobile, one type of proxying solution is an HTTP tunnelling component like [localtunnel](https://github.com/localtunnel/localtunnel). You run a local API on a port like 3000, then expose that port to the internet. The tunnel then prints details of requests that pass through it.

```bash
npx localtunnel --port 3000 --print-requests
```

Typically, you have to work with generated host names like *https://some.generated.api.hostname.lt* and there may be limitations on the HTTP request details you can view. Instead, I prefer to take more complete control over local URLs and visibility of HTTP requests.
...
You must deploy the certificate's root certificate authority file to the emulator or device. Most certificates, such as those that I generate for this blog using OpenSSL, use the *PEM* format and contain base64 encoded text that looks similar to this:
...
- Drag the certificate into an emulator.
- Email it to the device or emulator.
- Download it over HTTP.

For the MITM proxy root CA you can just browse to a URL of *http://mitm.it* from the device or emulator. Alternatively, use an HTTP server that exposes a folder on the host computer that contains the certificate file. The following command uses NGINX to share a host folder at *http://localhost* and use the NGINX *autoindex* feature to allow file browsing:

```bash
docker run -it --rm -p 80:80 -v ./fileshare:/var/lib/nginx/html:ro dceoy/nginx-autoindex
```

From a connected emulator or device you can open Chrome and type the IP address of the host. Click a certificate file and save it to the device's *Downloads* folder:
...
By default, devices and emulators will not be able to resolve this domain name. If you use an HTTP proxy however, domain name resolution for *api.authsamples-dev.com* runs on the host computer and succeeds.
...
I explained an Android HTTP debugging setup, to enable visualization of OAuth and API requests, which is an essential behavior for productive OAuth client development.
