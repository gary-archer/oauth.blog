...
After some research it turned out that this format of access token should only be received by Microsoft's APIs. The default format of access token is heavily used to secure Azure resources during Azure infrastructure deployments. This access token type always fails standards-based validation for your own APIs. I needed to use the second type of access token below:
