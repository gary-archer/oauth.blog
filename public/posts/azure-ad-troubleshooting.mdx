...
The SPA implements OAuth and OpenID Connect lifecycle events using Entra ID. The API validates Entra ID access tokens and uses claims-based authorization to protect its fictional data. The correct data is then rendered based on the signed in user:
...
![authorization request](../images/200/authorization-request.jpg?v=20260706)

The login response contains an authorization code:

![authorization response](../images/200/authorization-response.jpg?v=20260706)
...
After some research it turned out that this format of access token should only be received by Microsoft's APIs. This form of access token is used heavily to secure Azure resources during Azure infrastructure deployments. This access token type always fails standards-based validation for your own APIs. To resolve my problem, I needed to use the second type of access token below:
...
The access token contains a couple of technical user identifiers. The *oid* claim is the permanent user account ID, which is the same across all Entra ID applications. The *sub* claim is a unique user identifier private to the client. The access token contains the custom claims explained in the previous post to enable the API to implement authorization in a convenient way. I was unable to exclude personal data from the access token, even if I removed all Graph permissions from the API's registered client.
...
The SPA downloads personal data from the OpenID Connect userinfo endpoint, which uses Microsoft Graph. When using Entra ID, Graph is considered to be an Azure infrastructure resource. To access Graph, you must get a Graph access token. Therefore I use the [On Behalf Of Flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow) to swap the SPA's access token for a Graph access token. However, that flow requires a client credential, and you cannot safely store one in an SPA. 

Therefore, the SPA must route its Graph userinfo request via an API. The SPA calls the first URL below. The API performs the token exchange request and sends the Graph access token to the second URL to get userinfo. The API returns the resulting userinfo to the SPA:
...
Next the API sends the returned access token to the Graph endpoint to get the following userinfo, which it returns to the SPA, so that the SPA can render the signed in user's name:
...
Entra ID correctly supports the traditional iframe-based silent token renewal solution for SPAs. It does so by issuing SSO cookies with the *SameSite=none* property. In the SPA you can click *Expire Access Token* followed by *Reload Data* to force the API to return a 401 response. The SPA then runs an iframe redirect to refresh access tokens. Doing so avoids redirecting the whole window and impacting the user, after which the SPA receives a new access token.

![iframe token renewal](../images/200/iframe-token-renewal.jpg?v=20260706)

This flow is no longer reliable in all browsers though. Some browsers, like Safari with default settings, refuse to send third-party SSO cookies from an iframe.
...
Entra ID supports RP-initiated logout in the standard way, so I use the *oidc-client-ts* implementation to send an end session request. The SPA also continues to implement multi-tab logout by listening for updates to local storage from other browser tabs.

### 11. Extensible API Claims

The API follows this blog's <a href='api-authorization-design.mdx'>API Authorization Behaviour</a> and uses some values for authorization that are not contained in the Entra ID access token. These are included in the *ClaimsPrincipal* object injected into the API's business logic. The earlier <a href='updated-api-coding-key-points.mdx'>API Code Details</a> blog post explained the details.

### Where Are We?

I explained the types of technique you may have to use during OAuth integrations. Ideally, authorization server providers should follow OAuth standards, though many of them implement custom behaviour that makes portability non-trivial. Despite that, I always aim to use standards-based portable code in my OAuth-secured APIs and clients, to simplify migrations as much as possible.
