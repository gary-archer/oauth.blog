...
Previously we ran some <a href='api-automated-tests'>API Automated Tests</a>, and next we will deploy a local Docker-based log aggregation system. This enables logs to flow from APIs during development.
...
Then follow instructions to configure <a href='developer-ssl-setup'>Browser SSL Trust</a> for the following root certificate, so that we can run the Kibana UI later:
...
| Client | Behaviour |
| ------ | --------- |
| API | The <a href='api-architecture-node'>Final API</a> post explains how to run a local API setup |
| SPA | The <a href='how-to-run-the-react-js-spa'>Final SPA</a> post explains how to point to a local API |
...
- Next we will run some API <a href='api-technical-support-analysis'>Technical Support Analysis</a> queries
