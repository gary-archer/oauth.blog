...
Previously I explained this blog's <a href='api-automated-tests.mdx'>API Automated Tests</a>, and next I deploy a local Docker-based log aggregation system. This enables logs to flow from APIs during development.
...
Kibana runs using an HTTPS URL so you must create development certificates. Ensure that OpenSSL 3+ is installed and then run the following script:
