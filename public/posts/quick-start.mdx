...
The React SPA implements OAuth and OpenID Connect using the *Token Handler Pattern*. It uses SameSite cookies to transport short-lived access tokens to APIs, while also enabling pure SPA development and deployment.
...
The cross platform Electron React app runs on Windows, macOS and Linux. The [AppAuth-JS Library](https://github.com/openid/AppAuth-JS) implements OAuth and Open ID Connect, with logins that use the system browser:
...
The Android App is a Single Activity App that uses Kotlin and Jetpack Compose. The [Android AppAuth Library](https://github.com/openid/AppAuth-Android) implements OAurh and OpenID Connect, where the app receives login responses on HTTPS redirect URIs, registered as Android app links.
...
The [iOS AppAuth Library](https://github.com/openid/AppAuth-iOS) implements OAuth and OpenID Connect, where the app receives login responses on HTTPS redirect URIs, registered as iOS universal links.
