...
The Android app uses 2 online domains which I created as part of this blog's <a href='cloud-domain-setup.mdx'>Cloud Domain Setup</a>:
...
- First install the app on an emulator without running an HTTP proxy on the host computer so that deep linking registration succeeds.
- Start the HTTP proxy on the host computer and configure the emulator to use it.
