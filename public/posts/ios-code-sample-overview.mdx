...
Previously we covered <a href='android-coding-key-points'>Android Code Details</a> for our initial mobile app. Next we will provide the same OAuth secured app for iOS, using Xcode and SwiftUI, with AppAuth libraries and a modern coding style. The app connects to <a href='cloud-hosting'>HTTPS endpoints hosted in AWS</a>, so that you only need to run the mobile app's code to get a fully integrated solution.
...
The Mobile UI will provide the same <a href='api-journey-client-side'>API Client Journey</a> as for earlier Web UIs, with a list view as the home page:
...
- Next we will cover <a href='how-to-run-the-ios-code-sample'>How to Run the iOS Code Sample</a>
