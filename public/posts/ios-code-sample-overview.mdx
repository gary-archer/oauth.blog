...
The goal of the iOS code sample is to implement this blog's <a href='native-architecture-goals.mdx'>Native Architecture Goals</a> for an iOS app, to blend security with user experience and reliability. The app uses Swift technology for best native capabilities and OAuth to maximize login user experience choices. The app can make concurrent API requests with least privilege access tokens, and handle expiry events resiliently.
...
![code repo](../images/540/repo.jpg?v=20260627)
...
| Reactivate App | The user switches to another app and then re-runs the app from the home screen via the app's launch icon. |
| Deep Link | The user runs the app and then runs a deep link from an email. |

### Where Are We?

I have summarized the app's functionality and some reliability issues to overcome. Since the app implements OAuth and OpenID Connect in a standard way, it could be updated to support many other forms of user authentication, with zero code changes.
