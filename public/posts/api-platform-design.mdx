...
APIs should have a predictable way to deal with non-functional requirements like security, error handling and logging. First you need to understand the behaviors you want and then implement the foundational work. If you follow modern design patterns you should be able to separate concerns to implement those behaviors with clean code.
...
The API hosting platform should provide features to keep your APIs available, healthy and performant. You be able to integrate your preferred best of breed supporting components and write code without limitations. Hosting should also be portable so that you can deploy anywhere. My preferred hosting model for APIs is to use a cloud native platform like Kubernetes.
...
In the final theme of this blog I deploy my final APIs to a cloud Kubernetes cluster that runs under my AWS account. On the web side of the architecture I use a <a href='cdn-static-content-delivery.mdx'>content delivery network</a>. More generally I mix and match hosting depending on my requirements. 
For low cost demo APIs that I host online I use <a href='serverless-api-overview.mdx'>Serverless APIs</a>. These are cheaper than Kubernetes clusters but meet my goals less well.
