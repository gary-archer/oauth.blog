...
In the previous post I explained <a href='basicspa-execution.mdx'>How to Run the Basic SPA Sample</a>. This post takes a closer look at HTTP messages in an end-to-end flow between the SPA, authorization server and API. These messages highlight some OAuth behaviours to run a basic authorization code flow, but also deal with error handling and cross origin requests.

### Development Domains

This blog provides an <a href='oauth-infrastructure-setup.mdx'>OAuth Infrastructure Setup</a> that enables early thinking about deployments. You reference the following logical domains on a development computer. Components that run locally use a fictional parent domain of *authsamples-dev.com*, to indicate that they are under development. Meanwhile, the authorization server uses a real internet hosted domain associated to my AWS account.
...
### How to View OAuth Messages

Most of the time I trace HTTPS messages using an HTTP development proxy rather than browser tools. This allows me to view traffic from both the SPA and API together. See the <a href='http-debugging-setup.mdx'>HTTP Proxy Setup</a> post if you are new to this type of tool:
...
![Configuration Download](../images/40/config-download.jpg?v=20251230)

### Step 2. SPA Initiates an Authorization Redirect

The SPA client needs to get an access token with which to call its APIs. The SPA therefore uses the *oidc-client-ts* library to trigger an authorization request that returns an access token. The security library first makes a cross origin HTTP request to get OpenID Connect metadata at the below URL. In some authorization servers you may need to explicitly enable CORS for the SPA's web origin:
...
![Metadata Download](../images/40/metadata-download.jpg?v=20260706)

The *oidc-client-ts* library then creates an authorization request URL containing OAuth and OpenID Connect parameters. Parameters include some randomly generated values, like the *state* parameter. The SPA sets its location to this URL, to cause a front channel browser redirect to Cognito's authorization endpoint.

![Authorization Request](../images/40/authorization-request.jpg?v=20260706)

Authorization servers contain an OAuth client registration containing configuration settings, and only allow the request if the SPA uses the correct *Client ID* and *Redirect URI*. The client uses the following scope parameters to request permissions to issue to the access token once user authentication completes:
...
### Step 3. User Authenticates at the Authorization Server
...
In some use cases, you can configure the authorization server to request user consent after user authentication completes. The user then grants third-party clients particular API permissions that clients request using the *scope* parameter. In this blog's examples, there are no third-party clients so I do not activate user consent. I represent APIs and clients built by the same organization, which owns the data.

### Step 4. An Authenticated User Session Begins
...
![User Session Cookie](../images/40/user-session-cookie.jpg?v=20251230)
...
### Step 5. SPA Receives an Authorization Response
...
![Authorization Response](../images/40/authorization-response.jpg?v=20260706)

The SPA receives the authorization response on its redirect URI, which can be a particular path, and the convention is to use a */callback* path. If the SPA has a stored state in session storage it asks the security library to process the authorization response. The library makes recommended OAuth checks like verifying that the response state parameter matches the request state parameter.

### Step 6. SPA Swaps the Authorization Code for Tokens
...
![Authorization Code Grant](../images/40/authorization-code-grant.jpg?v=20260706)

When possible, your OAuth clients should provide a credential before they receive tokens, but an SPA has nowhere secure to store such a credential. The SPA is therefore considered a *public client* and gets tokens in a less secure way than *confidential clients*.

The result of an OAuth grant request is a token response. I configure the authorization server to return access tokens with a short 15 minute expiry. Before accepting tokens, the security library cryptographically validates the ID token. 

![Token Response](../images/40/token-response.jpg?v=20240727)

An OAuth flow for SPAs should avoid returning refresh tokens to the browser, since it is a long-lived credential and the browser has nowhere secure to store refresh tokens. Yet, at the time of writing, AWS Cognito does not allow the refresh token to be disabled.

### Step 7. Proof Key for Code Exchange is Used

PKCE is an extension to the original *authorization code flow* and involves the following additional steps, which security libraries implement. It protects against some types of interception and injection attacks against authorization codes. 

- Client generates a runtime secret (the *code_verifier*) before issuing an authorization redirect.
- Client stores the code verifier, such as in session storage.
...
### Step 8: SPA Handles Login Errors
...
```json
{
    ...
    "oauth": {
        ...
        "redirectUri":      "https://www.authsamples-dev.com/spa/callback",
...
![Authorization Response Error](../images/40/authorization-response-error.jpg?v=20260706)
...
### Step 9. View Access Token Claims
...
}
{
  ...
  "version": 2,
  "client_id": "hje94a2jj3lgkobkh57ikenhh",
...
- **scope**: Scopes can represent business areas within your APIs.

The authorization server should also issue an *audience (aud)* claim, which can be an array, to represent one or more related APIs. In larger API deployments, the *scope* and *aud* claims provide mechanisms to restrict the APIs that accept the access token. The access token can also contain custom claims that the API uses for its business authorization. I say much more about scopes and claims later in subsequent posts.

### Step 10. View ID Token Claims
...
### Step 11. SPA Downloads User Info

Next, the *oidc-client-ts* library sends the access token to the authorization server's userinfo endpoint, to get additional details for the user. I always code apps to receive personal data from this endpoint, rather than from the ID token. The frontend uses this API response to display the user's name details, to help prevent unintentional disclosure of Personally Identifiable Information (PII).
...
### Step 12. SPA Calls API with Access Token
...
### Step 13. API Validates Access Token
...
### Step 14. API Returns Error Status Codes
...
```json
{
  "utcTime": "2026-03-29T13:36:19.166Z",
  "path": "/api/companies",
  "method": "GET",
  "statusCode": 500,
  "error": {
    ...
    "clientError": {
      ...
      "id": 41671,
      "area": "BasicApi",
      "utcTime": "2026-03-29T13:36:19.210Z"
    },
    "serviceError": {
      "details": "ERR_JOSE_GENERIC, Expected 200 OK from the JSON Web Key Set HTTP response, URL: https://cognito-idp.eu-west-2.amazonaws.com/eu-west-2_CuhLeqiE9/.well-known/xxx.jwks.json",
      "stack": [
        "JOSEError: Expected 200 OK from the JSON Web Key Set HTTP response",
        "at fetchJwks (file:///home/gary/dev/oauth.websample1/api/node_modules/jose/dist/webapi/jwks/remote.js:29:15)",
        ...
        "at async flattenedVerify (file:///home/gary/dev/oauth.websample1/api/node_modules/jose/dist/webapi/jws/flattened/verify.js:74:15)",
        ...
        "at async AccessTokenValidator.execute (/home/gary/dev/oauth.websample1/api/src/host/oauth/accessTokenValidator.ts:44:28)",
        "at async ApiController.authorizationHandler (/home/gary/dev/oauth.websample1/api/src/host/controller/apiController.ts:39:24)"
      ]
    }
  }
}
...
### Step 15. API uses Claims for Authorization
...
### Step 16. API Returns 401 when the Token Expires
