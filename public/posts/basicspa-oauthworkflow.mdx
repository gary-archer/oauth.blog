...
The SPA client needs to get an access token with which to call its APIs, and needs to initiate user authentication to get one. The SPA uses the *oidc-client-ts* library to create an authorization request URL containing OAuth and OpenID Connect parameters. Parameters include some randomly generated values, like the *state* parameter. The SPA sets its location to this URL, to cause a front channel browser redirect to Cognito's authorization endpoint.

![Authorization Request](../images/40/authorization-request.jpg?v=20251230)
...
![Authorization Response](../images/40/authorization-response.jpg?v=20251230)

The SPA receives the authorization response on its redirect URI, which can be a particular path, such as */callback*, or you can use the SPA's base URL. If you use the base URL, some web technology stacks issue an extra redirect if the redirect URI does not include a */* suffix.

If the SPA has a stored state in session storage it asks the security library to process the authorization response. The library makes recommended OAuth checks like verifying that the response state parameter matches the request state parameter.

### Step 7. SPA Swaps the Authorization Code for Tokens
