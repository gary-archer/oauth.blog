...
The SPA receives the response on its redirect URI, which can be a particular path, such as */callback*, or you can use the base URL of the SPA. If you use the base URL, many web technology stacks will issue an extra redirect if the redirect URI does not include a */* suffix.
...
The result of all grant messages is to receive tokens. I configure the authorization server to return access tokens with a short 15 minute expiry. Before accepting tokens, the security library cryptographically validates the ID token. 
