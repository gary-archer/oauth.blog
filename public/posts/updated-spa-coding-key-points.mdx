...
The updated SPA continues to use vanilla TypeScript code and this blog's <a href='web-technology-setup.mdx'>Web Technology Setup</a>. That post explains how the SPA's entry point is an *index.html* file that references bundles. The development setup uses the rollup bundler to build modern ECMAScript bundles and an Express HTTP server to serve them. The web development setup also combines a restrictive content security policy with live reloading.
...
- *https://www.authsamples-dev.com/spa/callback?code=xxx&state=yyy*
...
```json
{
    ...
    "oauth": {
        ...
        "clientId":               "1m4c53b8uup5qkqvdn4l0e0fb5",
        "redirectUri":            "https://www.authsamples-dev.com/spa/callback",
        "postLogoutRedirectUri":  "https://www.authsamples-dev.com/spa/logoutcallback",
...
I also extended the SPA's startup handling of OAuth callbacks, to handle logout responses before login responses. The SPA receives the post logout response at a */spa/logoutcallback* path, and then redirects to the SPA's hash-based location of *#loggedout*.
...
public async handleOAuthCallback(): Promise<void> {
    ...
    if (pathname.endsWith('logoutcallback')) {
        history.replaceState({}, document.title, '#loggedout');
        return;
    }
