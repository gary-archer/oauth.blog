...
Even if the cookie settings was fixed, this is also the behavior in the Safari browser or Chrome incognito mode. These browsers drop all third party cookies except for certain exceptions, like top level redirects. Therefore, the updated code example correctly represents the future state of iframe based token renewal.
...
I explained the code to complete the traditional SPA flow, with in-memory storage of access tokens, and the SSO cookie used for token refresh. The use of iframes was a successful technique, but in 2021 it is no longer considered secure or reliable.
