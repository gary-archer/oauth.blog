...
The SPA's OAuthClient class therefore uses some updated *oidc-client-ts* library settings:
...
Even if the cookie behaviour was fixed, this is also the behaviour in the Safari browser or Chrome incognito mode. These browsers drop all third party cookies except for certain exceptions, like top level redirects. Therefore, the updated code sample correctly represents the future state of iframe based token renewal.
...
For best reliability, you could also store a refresh token in local storage and avoid the use of iframe-based access token refresh completely. Yet local storage is the least secure way to manage tokens in the browser, with greatest XSS threats. When you also store refresh tokens, you make threats much worse, since an attacker can potentially use a refresh token to get new access tokens without needing to supply a client credential.
