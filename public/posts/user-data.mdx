...
After an OAuth migration, APIs will by default receive access tokens containing only the generated subject claim. This might be a UUID that is not useful to APIs that operate on business data. Instead, you typically need to provide a more useful user identity in access tokens. This blog's API code samples therefore include the *manager_id* claim in access tokens:
...
| -------- | ----------- |
| Existing User Migration | A migration process reads existing users in bulk from an existing user data source and imports them into the authorization server. |
| Administrator User Creation | An administrator creates users and assigns attributes across multiple user data sources. |
| Self-Sign-Up | A user signs up and provides proof of their identity, after which attributes save to multiple user data sources. |
| Profile Edits | A user edits their profile in an application form and data is saved to multiple user data sources. |
...
That is, frontends may need to get user attributes from both the authorization server and their APIs. Although this may seem to add complexity, the separation prevents deeper problems like needing to frequently remodel authorization server user account data.

### Where Are We?

I explained the main design choices when storing user attributes in an OAuth architecture. In mature systems, a clean separation between core identity fields and business-specific attributes can provide the best manageability for APIs and clients.
