...
The web host returns a number of [recommended security headers](https://owasp.org/www-project-secure-headers/), the most important of which is a restrictive content security policy. During development the SPA can only interact with resources from its web host, the trusted authorization server and trusted APIs.
