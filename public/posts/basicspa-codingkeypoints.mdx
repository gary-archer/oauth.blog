...
The SPA uses this blog's <a href='web-technology-setup.mdx'>Web Technology Setup</a>. That post explains how the SPA's entry point is an *index.html* file that references bundles. The development setup uses the rollup bundler to build modern ECMAScript bundles and an Express HTTP server to serve them. The web development setup also combines a restrictive content security policy with live reloading.
...
- *https://www.authsamples-dev.com/spa/callback?error=invalid_request&state=yyy*

The SPA uses a redirect URI of *https://localhost/spa/callback*. Even though the initial SPA uses hash-based routing, the callback path should use a path, since many authorization servers do not allow redirect URIs with hash fragments. The SPA first detects an authorization response and then asks the *oidc-client-ts* library to process it. The response handling code then performs the following actions:
...
- A user navigates back after login and then navigates forward to return to the app.
- A user bookmarks the authorization server's login page.
- A user pastes malicious authorization responses into the application.
