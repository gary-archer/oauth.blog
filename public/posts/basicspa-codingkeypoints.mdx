...
The SPA's deployed HTML is minimal. The application's code builds to JavaScript bundle files that the browser can execute. The browser downloads the *indeex.html* file which triggers the download of CSS files and JavaScript bundles. At runtime, JavaScript creates HTML elements within the *root* element.
...
The runtime markup uses CSS classes like *row*, *col-* and *card* from the [Bootstrap Grid System](https://getbootstrap.com/docs/5.3/layout/grid/), which provides an intuitive way to arrange content into rows and columns, where card elements have their own inner rows and columns. The layout capabilities enable me to present the companies view, which is usually a list, as a card based layout on mobile layouts:
