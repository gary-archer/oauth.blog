...
| CSS | I use [Bootstrap](https://getbootstrap.com/docs/5.3/getting-started/introduction/) to control layout in a mobile first manner, and for most styling. |
...
For this blog's early SPAs I use the webpack development server to serve static content in a productive manner. I configure a local HTTPS setup with the following settings and use hot reloading when the frontend code changes, as is common in SPA development.
...
For this blog's early SPAs I use Bootstrap, as a basic CSS solution. More cutting edge CSS solutions can require many webpack plugins, which I want to avoid, to keep the focus on TypeScript code. Bootstrap provides a technically simple [Grid System](https://getbootstrap.com/docs/5.3/layout/grid/) for arranging content into rows and columns, where card elements have their own inner rows and columns. The layout capabilities enable me to present the companies view, which is usually a list, as a card based layout on mobile layouts.

<div className='smallimage'>
    <img src='../images/50/mobile-companies.jpg' alt='mobile companies' />
</div>
