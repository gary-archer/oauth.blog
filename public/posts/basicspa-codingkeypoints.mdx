...
An SPA uses a redirect URI of *https://localhost/spa/callback*. Even though the initial SPA uses hash-based routing, the callback path should use a path, since many authorization servers do not allow redirect URIs that contain hash fragments. The SPA first detects an authorization response and then asks the oidc-client-ts library to process it. The response handling code then performs the following actions:
...
- Users who click the back button after login and then return to the app.
