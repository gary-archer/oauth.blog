...
The <a href='basicspa-oauthworkflow.mdx'>SPA and API OAuth Messages</a> post explained the initial code sample's HTTPS messages. Next, I summarize the base setup and explain key points of the OAuth integration.

### SPA Code

The SPA downloads the following types of static resources to the browser.

| Area | Implementation Details |
| ---- | ---------------------- |
| HTML | I use a single static *index.html* page and its DOM elements are updated dynamically. |
| CSS | I use [tailwindcss](https://tailwindcss.com/) as a utility CSS framework, to control layout and appearance. |
| JavaScript | I use TypeScript code that gets compiled to JavaScript bundles when it runs in the browser. |

The SPA uses only a handful of external dependencies, expressed in its *package.json* file. The most interesting of these is the *oidc-client-ts* library, which provides a JavaScript OAuth and OpenID Connect implementation.
...
The SPA's deployed HTML is minimal. The application's code builds to JavaScript bundle files that the browser can execute. The browser downloads the *index.html* file which triggers the download of CSS files and JavaScript bundles. At runtime, JavaScript creates HTML elements within the *root* element.
...
I use the [Node.js Express HTTP Server](https://expressjs.com/en/) as the web host, from which the browser downloads web static content over HTTPS. The most essential code to do so is shown below. The SPA web host is a technically simple component. In particular, the web host should never come into contact with sensitive data. 
...
The static content gets loaded from the *dist* folder, which contains the following files, which the rollup bundler writes. During development, whenever I change code in an editor, the web resources get updated.
...
The web development server also implements live reloading when I edit the SPA's TypeScript code or CSS styles, to update the *dist* folder and refresh the page. I explain much more about web mechanics, like rollup builds and live reloads, in the <a href='web-technology-setup.mdx'>Web Technology Setup</a> post. Ultimately, rollup transpiles the SPA's code to modern ECMAScript code that executes in the browser.
...
Before running a login redirect, the *OAuthClient* stores the SPA's current location in session storage and then issues a full screen redirect. In this blog, I always use full-screen redirects, since that is the most standard way to implement logins with fewest user experience issues.
...
- Technical users who paste malicious authorization responses into the application.
...
The SPA now successfully gets an access token from the *oidc-client-ts* library and uses the browser's *fetch* library to send the access token to the API.
...
The SPA uses the [Mustache Template Library](https://github.com/janl/mustache.js/) to bind received API data to the SPA's DOM elements. This ensures that potentially dangerous input is escaped, to help prevent malicious code execution.
...
The SPA uses styles from the *tailwindcss* framework. I use layout styles to arrange elements into rows and columns. I use appearance styles when rendering UI elements like buttons and labels. The application uses an *app.css* file to import tailwindcss. The build process produces a final *app.css* with the parts of the framework that my TypeScript code references.
