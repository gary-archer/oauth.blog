...
I use a simple Node.js Express server as the web host, from which the browser downloads web static content over HTTPS. The most essential code to do so is shown below. The SPA web host is a technically simple component. In particular, the web host should never come into contact with sensitive data. 
