...
The goal of the Android code sample is to implement this blog's <a href='native-architecture-goals.mdx'>Native Architecture Goals</a> for an Android app, to blend security with user experience and reliability. The app uses Kotlin technology for best native capabilities and OAuth to maximize login user experience choices. The app can make concurrent API requests with least privilege access tokens, and handle expiry events resiliently.
