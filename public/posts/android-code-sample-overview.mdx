...
Previously we covered our <a href='ios-https-debugging.mdx'>iOS HTTPS Debugging Setup</a> and next we will describe the behaviour of our main Android code sample, which is a *Single Activity App* coded in Kotlin. The app connects to <a href='cloud-hosting.mdx'>HTTPS endpoints hosted in AWS</a>, so that you only need to run the mobile app's code to get a fully integrated solution.
...
Our Mobile UI will provide the same <a href='api-journey-client-side.mdx'>API Client Journey</a> as earlier Web UIs, with a list view as the home page:
...
The sample also implements <a href='logout.mdx'>Open ID Connect RP-Initiated Logout</a>, to remove the authorization server’s session cookie. In a real-world app a logout capability enables you to test data access for different users with different settings or permissions.
...
- Next we will cover <a href='basicandroidapp-execution.mdx'>How to Run the Android Code Sample</a>
