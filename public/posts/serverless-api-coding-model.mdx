...
When you run *npm start* to run the API for development, a helper module runs two child processes. The first child process runs rollup in watch mode. The second child process runs Serverless Offline, which provides HTTPS entry points for the API, translates incoming requests to the lambda format and then calls lambda handlers, to invoke the bundled code.
