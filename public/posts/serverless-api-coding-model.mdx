...
During creation of a lambda instance I wire up middleware objects and register objects with a container that provides dependency injection features:
...
Serverless offers only limited control over log output, such as use of a *SLS_DEBUG=** environment variable which I rarely use, since it adds a lot of noise. To ensure that I look at my own preferred output I instead write logs to a file during development or as bare JSON to support log aggregation in AWS.
...
Use of a log file enables local verification of log aggregation. If I need to add additional log statements, I usually avoid *console.log* statements and also append such output to the log file instead.
