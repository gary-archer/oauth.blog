...
Previously I described this blog’s <a href='serverless-api-deployment'>Serverless API Deployment</a>. Next we will explain how we have coded this blog’s lambda based APIs, to include the behaviours from this blog’s <a href='api-journey-server-side'>API Journey – Server Side</a> blog post.
...
Previously we explained this blog’s approach to <a href='jwt-access-token-validation'>JWT Access Token Validation</a>, which involves caching the token signing public keys in-memory. For the Serverless API I needed to provide a custom key retriever to the JOSE library:
...
The API authorizes using this blog’s <a href='api-authorization-design'>API Authorization Behaviour</a>. This involves looking up and caching the following extra claims when an access token is first received.
...
The earlier <a href='updated-api-coding-key-points'>Updated API Code Sample</a> blog post described how this was implemented in Node.js. To cache extra authorization values not included in the access token, the *AwsCache* class is again used.
...
- Next we will discuss some future <a href='cloud--native-updates'>Cloud Native Updates</a> to this blog
