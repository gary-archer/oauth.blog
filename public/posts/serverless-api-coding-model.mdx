...
When you run *npm start* to run the API for development, a helper module runs two child processes. The first child process runs the rollup bundler in watch mode. The second child process runs Serverless Offline, which provides HTTPS entry points for the API, translates incoming requests to the lambda format and then calls lambda handlers, to invoke the bundled code.
...
During the first API request, AWS or Serverless Offline creates a *LambdaInstance*, which is used to process that request and multiple subsequent requests. Lambda handlers which have an equivalent role to REST controller endpoints. During a request, the lambda resolves a service object that implements business logic. Cross cutting concerns execute before and after the lambda base handler.
...
During creation of a lambda instance I create an [InversifyJS](https://github.com/inversify/InversifyJS) container and register objects with it, including middleware that handles cross cutting concerns:
...
InversifyJS enables constructor injection, to benefit code clarity, extensibility and testing:
...
The Serverless lambdas continue to use this blog's approach to <a href='jwt-access-token-validation.mdx'>JWT Access Token Validation</a>. The Serverless API requires a valid access token on every request, as part of a zero trust architecture.
