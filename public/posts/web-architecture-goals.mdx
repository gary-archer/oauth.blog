...
Sites like [The Modern Web](https://modern-web.dev/) promote technologies that are easier to manage from a development viewpoint than older website solutions. Developers no longer need to battle with a web backend that dictates frontend behaviour. 

![The Modern Web](../images/10/modern-web.jpg)

OAuth also improves productivity, since it enables web apps to externalise data protection to APIs, and user authentication to the authorization server. As a result, web developers should never need to implement backend security, which greatly simplifies their responsibilities.
...
If an app has XSS vulnerabilities, then an attacker may be able to access sensitive data in exactly the same way as the real web app. Therefore, reducing the impact of XSS exploits is now a major consideration when designing web solutions.

### Best Login Usability

The authorization server can provide multiple ways for users to sign in to an SPA, and this can include options that are both user friendly and secure. In addition, an SPA should be in full control of usability and support features like multi-tab browsing.

Web applications should cope elegantly with API challenge responses that use HTTP status codes of 401 and 403. The frontend guides the user, rather than issuing an abrupt redirect. To enable that, JavaScript code should issue all API requests for sensitive data.
