...
In <a href='user-data.mdx'>User Data Management</a> I explained how mature real-world systems typically store *core user attributes* separately to *volatile business user attributes*. You also need to deal with both types of data when you authorize requests in your API code.
...
Real-world authorization usually includes a number of fine-grained permissions. An access token identifying the user is sent to one or more APIs, which might need to enforce business rules such as these:
...
| Scope Checks | Boundary checks to ensure an access token's validity for a particular area of data or high-privilege operation. |
| Claims-Based Authorization | Detailed permission checks against resources, using claims and other fine-grained permissions to apply business rules. |
...
You can issue any data values as access token claims and the authorization server resolves them per user, so claims are much more flexible than scopes. In many use cases, APIs need dynamic runtime values to authorize correctly. OAuth enables you to design solutions where the authorization server provides APIs with runtime claims, whose values may originate from trusted third-party systems.
...
When this blog's APIs first receive a particular access token, they use the *manager_id* to look up the following extra claims from a trusted source, like a database. Therefore, these values also derive from access token claims, and the API authorization code uses them in an identical way to true claims from the access token:
