...
In our previous post we described our <a href='net-core-api-oauth-integration'>.NET API OAuth Integration</a>. We will now drill into some final implementation details.
...
Logging classes writes to a JSON log file and, if the <a href='log-aggregation-setup'>Log Aggregation Setup</a> is followed, logs will automatically flow to Elasticsearch:
...
The API implements our <a href='error-handling-and-supportability'>Error Handling and Supportability</a> design. By default all errors escape to the global exception handler, whose role is to log errors and return an error response to the caller:
...
- Next we will implement our <a href='java-spring-boot-api-overview'>API Architecture in Java</a>
