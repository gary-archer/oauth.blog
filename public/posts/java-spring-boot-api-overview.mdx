...
Previously I explained the <Link href='/posts/final-nodeapi-coding-key-points'>Final .NET API Code Details</Link>, including some behaviours from this blog’s <Link href='/posts/api-journey-server-side'>API Journey – Server Side</Link> blog post. Next I provide an equivalent API in Java and Spring.
...
- I explain this blog's <Link href='/posts/spring-boot-api-oauth-integration'>Java API OAuth Integration</Link>.
