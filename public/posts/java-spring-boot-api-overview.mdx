...
Previously I explained the <a href='final-nodeapi-coding-key-points'>Final .NET API Code Details</a>, including some behaviours from this blog’s <a href='api-journey-server-side'>API Journey – Server Side</a> blog post. Next I provide an equivalent API in Java and Spring.

### Final API Code Sample Overview

This blog's API has a couple of simple endpoints that return mock data for a money-based theme of investments. The API aims to be visual and serves this blog’s apps, such as the <a href='how-to-run-the-react-js-spa'>Final SPA</a> below. Running both together ensures that the API is client-focused and enables you to focus on API logs that the client generates.
...
The API runs over SSL and when you run it for the first time it downloads some development certificates. To prevent SSL trust problems from tests or the browser you may need to <a href='developer-ssl-setup'>Configure Java and Browser Trust</a> for the root certificate at the following location. 
...
- <a href='final-spa-overview'>Final SPA</a>
- <a href='final-desktop-sample-overview'>Final Desktop App</a>
- <a href='android-code-sample-overview'>Final Android App</a>
- <a href='ios-code-sample-overview'>Final iOS App</a>
...
- <a href='api-authorization-design'>API Authorization Design</a>
...
Optionally, you can follow the <a href='log-aggregation-setup'>Log Aggregation Setup</a> to run a Docker-based deployment of the Elastic Stack. You can then run <a href='api-technical-support-analysis'>Technical Support Queries</a> against the API's logs. This would be useful in a production system, to make sense of large volumes of API requests.
...
- I explain this blog's <a href='spring-boot-api-oauth-integration'>Java API OAuth Integration</a>.
