...
After cloning the sample code, run the following command to build and run the app.

```bash
./start.sh
```

This command starts a child window that builds code for the main side of the app, then builds code for the renderer side of the app in watch mode, then runs a live reload server that notifies the frontend after renderer code changes.
...
The protocol scheme is also used as an OAuth private URI scheme, with which the desktop app receives OAuth repsonses and other deep links from the desktop operating system.
...
One solution is to avoid logout redirects and use the OpenID Connect *prompt=login* parameter when you sign in. Instead, I set the app's redirect URIs to external web pages, at the following URLs. These pages also improve a little on the default blank browser page that the user sees after login.
