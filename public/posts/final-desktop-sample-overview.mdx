...
Previously I explained some <a href='desktop-app-coding-key-points.mdx'>Code Details</a> for this blog's initial desktop app. Next I add some missing features to complete session management, harden security, and improve usability.

### Code Sample Goal

The goal of the final desktop app is to implement this blog's <a href='native-architecture-goals.mdx'>Native Architecture Goals</a> for an Electron desktop app, to blend security with user experience and reliability. The app uses TypeScript for productive cross platform development, and OAuth to maximize login user experience choices. The app can make concurrent API requests with least privilege access tokens, and handle expiry events resiliently.
...
| Feature | Description |
| ------- | ----------- |
| Technology Modernization | I upgraded from plain TypeScript views to a React app that uses JSX syntax and path-based navigation. |
| Deep Linking | The app receieves login responses using private use URI scheme based redirect URIs, to enable desktop deep links. |
...
![repo](../images/440/repo.jpg?v=20260607)

### Run the Code Sample

Run the following bash script to start the desktop app:

```bash
./start.sh
```

The script starts a child window that builds code for the main side of the app, then builds code for the renderer side of the app in watch mode, then runs a live reload server that notifies the frontend after renderer code changes.

![child windows](../images/440/child-window.jpg)
...
The app also implements OpenID Connect RP initiated logout, which presents a similar completion page. The logout solution enables the use of single sign-on, while also enabling logins for multiple test users.

![logout web page](../images/440/logout-web-page.jpg)

### OAuth Configuration Changes

The final desktop app's configuration no longer requires HTTP ports, and now uses login and logout completion pages as redirect URIs:
...
```json
{
    "app": {
        ...
        "protocolScheme": "x-authsamples-desktopapp"
    },
    "oauth": {
...
The updated desktop app uses a [custom protocol](https://www.electronjs.org/docs/latest/api/protocol) of *x-authsamples-desktopapp*. The renderer process uses the scheme to download web files from the main process. Using a custom protocol mitigates file-based XSS attacks from the renderer process.

![custom protcol](../images/440/custom-protocol.jpg)

The protocol scheme is also used as an OAuth private use URI scheme, with which the desktop app receives OAuth responses and other deep links from the desktop operating system.

### External Web Redirect URIs
...
I set the app's redirect URIs to external web pages, at the following URLs.
...
If you use your browser's *View Source* option you can see that the pages simply forward the login response to the app's private use URI scheme callback endpoint:
...
### Private Use URI Scheme Browser Prompts
...
The following screenshots show the appearance of the user prompt in the four main desktop browsers. The user can select the *Always Allow* option to avoid a prompt on every login:
...
The desktop app stores OAuth tokens in a text file. I use [Electron safeStorage](https://www.electronjs.org/docs/latest/api/safe-storage) to create an operating system encryption key that no other app or user can access. On macOS the encryption key is saved to the Keychain. On Windows and Linux the key is less visible. Windows uses the *DPAPI* subsystem, and my Ubuntu Linux system uses the *GNOME libsecret* subsystem. Further details on the underlying security are discussed in [this online thread](https://github.com/microsoft/vscode-discussions/discussions/748).
...
In some use cases, you can use deep linking to bookmark screens. For example, you might send the user an email with a URL link. Note however that email clients may consider the private use URI scheme's prefix suspect, and block this type of deep link:
...
I updated the desktop code sample with some final features and explored desktop logins using private use URI schemes. Login usability remains unnatural, due to use of the disconnected browser.
