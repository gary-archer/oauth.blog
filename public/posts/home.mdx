...
This blog focuses on productive and reliable code setups when developing APIs, browser based apps and mobile apps within a distributed architecture. Although the use of modern technology stacks may seem straightforward, end-to-end flows require technical foundations before they are production-ready.

### API Technical Foundations

When APIs and API clients run in production, they need a number of technical qualities. I usually start with those listed below.
...
I first explain my thoughts on the most important goals and requirements. I then  explain some techniques to meet them, with setups that anyone new to OAuth can use to easily run end-to-end flows with secured APIs and clients. 

I only use basic OAuth security, but once you have the right setup and separation you are well-placed to implement more advanced use cases. In addition to OAuth integration I explain some lessons I've learned to enable resilient API and client journeys:
...
The samples cover a developer journey that starts with a security-first setup. I use OAuth to secure APIs and clients using standards-based, portable code. During integration I rehearse error, expiry and concurrency conditions, to ensure reliability.
