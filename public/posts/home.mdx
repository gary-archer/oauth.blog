...
This blog follows a developer journey that starts with important goals and requirements. Some of these are not security-related, like the desire to use modern technology. I explain techniques that I use, to enable secure, reliable integrations with a productive developer setup. I also explain some generic approaches to provide future-proof systems:
...
This blog's frontend code samples use a security-first and business-like setup on a development computer. I use OAuth to secure APIs and clients with standards-based, portable code. During integration I rehearse error, expiry and concurrency conditions, to ensure reliability.
...
I summarised blog goals and the first objective is a code sample that integrates a single page application (SPA) and API with an authorization server. Before beginning the initial code journey, I set the scene, to raise some important requirements.
