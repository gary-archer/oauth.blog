...
This blog explores productive and reliable code setups whe developing APIs, browser based apps and mobile apps. Although the use of modern technology stacks may fell straightforward, end-to-end flows require technical foundations before they are production-ready.

I find very limited material on these topics, so some years ago I decided to write this blog, to combine my experience as an architect with my experience as a coder. I start with goals and requirements, then demonstrate my preferred ways to achieve them using modern code. Of course there are many possible ways to build solutions from requirements, so you should treat my opinions only as suggestions.

### API Technical Qualities

Before most APIs and API clients can run in production, they need a number of technical behaviours:

| Quality | Description |
| ------- | ----------- | 
| Secure | Clients call APIs using message credentials containing user attributes that APIs use to protect business data. |
