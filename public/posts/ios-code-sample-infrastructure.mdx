...
Previously we described <a href='how-to-run-the-ios-code-sample'>How to Run the iOS Code Sample</a>, and next we will focus further on the infrastructure used by our OAuth solution.

### AWS CloudFront Domains

Our iOS sample uses 2 online domains, which we configured previously as part of our <a href='cloud-domain-setup'>Cloud Domain Setup</a>:
...
They are then included in one of this blog’s <a href='cdn-static-content-delivery'>CloudFront Distributions</a>, so that the pages are served over an HTTPS URL:
...
- Next we will describe some <a href='ios-code-sample-coding-key-points'>iOS Code Details</a>
