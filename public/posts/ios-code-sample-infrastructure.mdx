...
./security/developmentAssets/run.sh
```
...
- First install the app on a simulator without running an HTTP proxy on the host MacBook so that deep linking registration succeeds.
- Start the HTTP proxy on the host MacBook and configure the simulator to use it.
