...
To build the Electron app I use webpack to produce bundles for both the main and renderer sides of the application. Two webpack builds run, using the techniques from the earlier post on <a href='web-technology-setup.mdx'>Web Technology Setup</a> blog post, to produce the following ECMAScript bundle files from the code layout and the *node_modules* folder:
