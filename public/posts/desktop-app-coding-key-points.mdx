...
For Electron apps it would be unnatural to use the webpack development server, since the main process serves static content. Instead, I run the app during development from the built files in the *dist* folder. I therefore develop with all of the same web resources and asset loading behaviors as production users.
