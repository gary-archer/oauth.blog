...
The desktop app hosts an instance of the Chromium browser that runs the renderer process as an SPA. To build the Electron app I produce ECMAScript bundles for both the main and renderer sides of the application. The main side of the Electron app serves static content from the file system to the renderer process.

The renderer process uses this blog's <a href='web-technology-setup.mdx'>Web Technology Setup</a>. That post explains how the frontend's entry point is an *index.html* file that references bundles. The development setup uses the rollup bundler to build modern ECMAScript bundles. The web development setup also combines a restrictive content security policy with live reloading.

### Inter Process Communication

The app follows the [Electron Security Best Practice](https://www.electronjs.org/docs/latest/tutorial/security) to run the renderer process with least privilege. It therefore does not have permissions to perform OAuth operations like running an HTTP server or opening the system browser. Instead, to initiate this type of operation, the desktop app uses Electron's [Inter-Process Communication](https://www.electronjs.org/docs/latest/tutorial/ipc). I also proxy API requests via the backend to keep tokens out of the Chromium browser. The main process registers some IPC event handlers for the following event names:
...
The Electron design choice to use a browser to render frontend views leads to IPC events, which add significant code complexity. That complexity is not present in desktop applications that perform native rendering, or in mobile applications that use native views.
...
| ----- | ---------------- |
| ApiClient | A forwarder to initiate API operations. |
...
```typescript
private async callApi(method: string, url: string, dataToSend: any = null): Promise<any> {
...
Meanwhile the renderer's *ApiClient* uses the following generic routine to invoke an API operation via its IPC callback. This uses a standard algorithm to refresh access tokens or handle session expiry:
...
When there is no token yet or token refresh fails with an *invalid_grant* error, views receive a *login_required* error and terminate API requests in a controlled manner. This error code is treated specially and results in a redirect to the *Login Required View*, which saves the previous location, to return to later:
...
The initial desktop app stores its tokens in memory. Every time the app restarts the user must re-authenticate. I show how to improve this user experience in the final desktop code sample.

### Handling Re-Entrancy

For a reliable solution, you must handle multiple login attempts and the following events:
...
I explained some Electron mechanics and the integration of the AppAuth-JS library, to secure the cross-platform desktop app. Next, I add some missing features, in this blog's final desktop code sample.

### Next

- I provide a <a href='final-desktop-sample-overview.mdx'>Final Desktop Code Sample</a> with some additional OAuth-related behaviours.
