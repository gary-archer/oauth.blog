...
To build the Electron app I use webpack to produce ECMAScript bundles for both the main and renderer sides of the application. Two webpack builds run, using the techniques from the earlier <a href='web-technology-setup.mdx'>Web Technology Setup</a> blog post. The main build produces a *main.bundle.js* file and the renderer build produces *app.bundle.js* and *vendor.bundle.js* files.
...
For Electron apps it would be unnatural to use the webpack development server, since the main process serves static content. Instead, I run the app during development from the built files in the *dist* folder. I therefore develop with all of the same web resources and asset loading behaviours as production users.
