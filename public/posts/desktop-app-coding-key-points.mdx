...
For Electron apps it would be unnatural to use the webpack development server. Instead, I run the app during development from the built files in the *dist* folder. I therefore operate with all of the same assets and asset loading behaviors as production users. I build the renderer app in watch mode, so that whenever HTML, CSS or JavaScript code changes, the content in the *dist* folder updates. There is no automatic reloading, but I can easily use the *View / Reload* Electron menu option to reload the renderer app.
