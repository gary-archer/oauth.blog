...
Previously I articulated some <a href='cloud-hosting.mdx'>Cloud Deployment Goals</a>. Next I show how a developer can get started with a cloud platform to deploy applications with working OAuth flows. I use AWS and start with creating domains and external base URLs on which to expose APIs, web static content and an authorization server to the outside world.
...
| *authsamples-dev.com* | A local computer domain that I use when developing API code. |
| *authsamples-k8s.com* | A cloud domain that runs in AWS and uses Kubernetes hosting for the API side of the architecture. |
| *authsamples-k8s-dev.com* | A local computer domain that I use to rehearse local Kubernetes deployments. |
...
AWS CloudFront and AWS Cognito both require certificates issued in the North Virginia region. I expressed each subdomain as a certificate subject alternative names and also selected the *Create records in Route 53* option to enable certificate manager to verify my ownership of the domain.
...
The API side of this blog's architecture can also run in a Kubernetes environment. In AWS I can use the [Elastic Kubernetes Service (EKS)](https://docs.aws.amazon.com/whitepapers/latest/overview-deployment-options/amazon-elastic-kubernetes-service.html) to create a managed cluster. This is a more expensive hosting option, so I avoid permanently running Kubernetes clusters for my online code samples and use serverless hosting instead.
...
I showed how to create low cost subdomains in a cloud system by following equivalent DNS and SSL technical steps to those that this blog uses on a development computer. You can do so using serverless building blocks, which are cheaper for my online code samples. For a real company you could use cloud native building blocks to get finer control over technical behaviours.
