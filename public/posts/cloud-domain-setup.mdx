...
I also use a *https://bff.authsamples.com* domain as a backend for frontend base URL for this blog's final SPA.
...
I then installed the [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) so that I can deploy applications and automate the creation of AWS infrastructure to provide HTTPS endpoints. I use the *aws* command to trigger deployments and use [aws login](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sign-in.html) when prompted, to authorize the AWS CLI.

### Create Cloud Domains

After activating the account I used the [Route 53](https://aws.amazon.com/route53/) DNS service to create my cloud domains. Each of these costs around 15 USD per year and provides low cost hosting for my hosted code samples.
