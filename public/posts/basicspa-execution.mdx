...
The project demonstrates a code separation to aim for. The API serves protected data to the frontend. The web application is entirely frontend-focused. Web developers no longer need to implement the type of backend logic that older website solutions required.
...
In the root folder, run the following commands to spin up the system. The deployment uses OpenSSL to create development HTTPS certificates in your preferred secrets folder. Other than that it mostly runs *npm install* and *npm start* for the SPA and API components:
...
Once user authentication completes, the SPA renders hard coded fictional data returned from the API. This data is secured using OAuth access tokens. The API uses [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS), to grant the SPA's precise domain the cross-origin permissions to call the API.
...
### Step 11: View Security Library Logs
