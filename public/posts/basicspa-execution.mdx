...
<div className='smallimage'>
    <img src='../images/30/code-layout.jpg?v=20260620' alt='Code layout' />
</div>
...
![Client settings 2](../images/30/client-settings2.jpg?v=20260706)
...
In the root folder, run the following commands to spin up the system. The first time you run one of this blog's code samples, the deployment creates <a href="developer-ssl-setup.mdx">OpenSSL Development Certificates</a> in your preferred secrets folder.
...
The first child terminal uses the rollup tool to build the SPA's TypeScript code into JavaScript bundles that the browser can execute. It then run a development web server on port 443 to serve HTML, CSS and JavaScript as web static content:

![Running SPA](../images/30/running-spa.jpg?v=20260620)

The second child terminal runs the API. On a development workstation you have to use distinct port numbers for the SPA and API. I use port 446 for the API, but in a real deployment it would use port 443.

![Running API](../images/30/running-api.jpg?v=20251229)

### Step 7: Sign In to the SPA

The *start.sh* script then invokes a browser at *https://www.authsamples-dev.com/spa*. The subpath gives the web domain space to grow, so that you could deploy multiple SPAs as micro frontends at different paths. Initially, the SPA attempts to load its home view and call the API to get data. However, the SPA does not have an access token yet so it redirects the user to a login required view:

![Login Required](../images/30/login-required.jpg?v=20251229)

The SPA never issues abrupt login redirects and instead guides the user, who can initiate the login. This provides good usability and also prevents the potential for redirect loops if you ever use incorrect OAuth configuration settings for the SPA or API.
...
To enable the SPA to successfully call APIs, this blog's early APIs implement [Cross Origin Request Sharing](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS), to grant the SPA's precise domain the cross-origin permissions to call the API.
...
The SPA's session management is incomplete in the initial code sample. Every time the user opens a new browser tab or the access token expires, the user has to sign in again. To simulate access token expiry for a browser tab, click *Expire Token* followed by *Reload Data*.
...
![HTTP debugging](../images/30/http-debugging.jpg?v=20260706)

### Step 12: View Security Library Logs

The SPA can report *oidc-client-ts* logs, to provide visualisation of its behaviours. Activate logs by adding a *#log=debug* query parameter to the browser's URL, then viewing the browser console. The following screenshot uses Google Chrome development tools. Note that I select the *Verbose* level to show debug messages and *Preserve Log* to maintain messages before and after login redirects.
