...
The SPA can report oidc-client-ts logs, to provide visualisation of its behaviors. Activate logs by adding a *#log=debug* query parameter to the browser's URL, then viewing the browser console. The following screenshot uses Google Chrome development tools. Note that I select the *Verbose* level to show debug messages and *Preserve Log* to maintain messages before and after login redirects.
