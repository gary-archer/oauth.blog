...
const authenticationMiddleware = new AuthenticationMiddleware(this.configuration.oauth);
...
export class CompanyRepository {
    ...
    public async getCompanyList(): Promise<Company[]> {
        ...
        const breakdown = this.logEntry.createPerformanceBreakdown('selectCompanyListData');
...
The API uses an OAuth filter class to implement access token validation and create a claims principal, in a similar way to the earlier <a href='updated-api-coding-key-points.mdx'>Updated API Code Details</a> blog post. The authentication middleware also adds the claims principal to the child container to make it resolvable, and enforces required scopes:
