...
The code for the advanced Node.js API consists of three main areas:
...
This is very standard, but the art of good error handling is to design good error objects that contain useful fields to both callers of the API and your technical support staff. Development error output has a productive and readable format that matches the rendering of production logs. Each log entry includes context like the current user, session, API and operation.
...
The API uses some techniques not seen in many example Node.js APIs. However, the design is very mainstream and commonly used in other API technologies, to enable a productive code setup. The code for this blog's .NET and Java APIs is almost identical.
