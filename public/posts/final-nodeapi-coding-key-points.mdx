...
The *HttpServerConfiguration* class manages controller resolution by first declaring metadata for HTTP routes:
...
```typescript
public async onException(request: Request, response: Response, next: NextFunction): Promise<void> {
