...
container.bind<OAuthFilter>(BASETYPES.OAuthFilter)
    .to(OAuthFilter).inRequestScope();
container.bind<CompanyService>(SAMPLETYPES.CompanyService)
    .to(CompanyService).inTransientScope();
```

For APIs with small dependency graphs, I reduce the likelihood of parallel requests impacting each other, by giving each request its own independent object instances. I use a request scope for REST specific objects, or a transient scope otherwise.
